#!/bin/bash
# ============================================================================
#  Unify Admin — Put all features under one sidebar
#  Replaces old sidebar in existing admin pages with AdminShell
#  Removes duplicate KYC (legacy compliance replaces it)
# ============================================================================
set -euo pipefail
...
# ── 1. Update AdminShell — remove Legacy section ────────────────────────────
log "Updating AdminShell — removing legacy links..."
sed -i '/sep.*LEGACY/d' components/admin/AdminShell.js
sed -i '/admin\/kyc.*Legacy/d' components/admin/AdminShell.js
sed -i '/admin\/market.*Market/d' components/admin/AdminShell.js
sed -i '/admin\/support.*Support/d' components/admin/AdminShell.js
# Add Market and Support under correct sections
sed -i '/admin\/treasury.*Treasury/a\  { href: "/admin/market", icon: "📈", label: "Market Data" },' components/admin/AdminShell.js
sed -i '/admin\/reports.*Reports/a\  { href: "/admin/support", icon: "💬", label: "Support Tickets" },' components/admin/AdminShell.js
ok "AdminShell.js — cleaned up"

# ── 2. Replace /admin/index.js (Dashboard with new sidebar) ─────────────────
log "Replacing /admin/index.js with unified dashboard..."
cat > pages/admin/index.js << 'ENDOFFILE'
...
ENDOFFILE
ok "pages/admin/index.js — unified with AdminShell"

# ── 3. Remove old duplicate KYC page (compliance replaces it) ────────────────
log "Removing old /admin/kyc.js (replaced by /admin/compliance)..."
rm -f pages/admin/kyc.js
ok "pages/admin/kyc.js removed"

# ── 4. Update /admin/users.js to use AdminShell ─────────────────────────────
log "Updating /admin/users.js to use AdminShell..."
...
ENDOFFILE
ok "pages/admin/users.js — uses AdminShell"

# ── 5. Update /admin/market.js to use AdminShell ────────────────────────────
log "Updating /admin/market.js..."
if [ -f pages/admin/market.js ]; then
  # Wrap existing content with AdminShell import
  sed -i '1s/^/import AdminShell from "..\/..\/components\/admin\/AdminShell";\n/' pages/admin/market.js 2>/dev/null || true
  ok "pages/admin/market.js — AdminShell import added"
fi

# ── 6. Update /admin/support.js to use AdminShell ───────────────────────────
log "Updating /admin/support.js..."
if [ -f pages/admin/support.js ]; then
  sed -i '1s/^/import AdminShell from "..\/..\/components\/admin\/AdminShell";\n/' pages/admin/support.js 2>/dev/null || true
  ok "pages/admin/support.js — AdminShell import added"
fi

# ── 7. Update /admin/assets/index.js to use AdminShell ──────────────────────
log "Updating /admin/assets/index.js..."
if [ -f pages/admin/assets/index.js ]; then
  sed -i '1s/^/import AdminShell from "..\/..\/..\/components\/admin\/AdminShell";\n/' pages/admin/assets/index.js 2>/dev/null || true
  ok "pages/admin/assets/index.js — AdminShell import added"
fi

# ── GIT + DEPLOY ─────────────────────────────────────────────────────────────
log "Committing and deploying..."
git add -A
git commit -m "feat: unify all admin under one sidebar

- /admin/index.js now uses AdminShell with full navigation
- /admin/users.js rebuilt with AdminShell + search
- /admin/kyc.js removed (replaced by /admin/compliance)
- AdminShell: removed Legacy section, added Market + Support
- All admin pages accessible from one sidebar"
...
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  Admin unified! All features under one sidebar.${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "  Sidebar now shows:"
echo -e "    OVERVIEW:    Dashboard, Users, Assets"
echo -e "    COMPLIANCE:  KYC/KYB Queue, Travel Rule"
echo -e "    ASSETS:      Listing Moderation, Registry, Contracts, Vault"
echo -e "    FINANCIAL:   Treasury & Revenue, Market Data"
echo -e "    SECURITY:    Security Center"
echo -e "    REPORTING:   Regulatory Reports, Support Tickets"
echo ""
echo -e "  Removed: /admin/kyc (replaced by /admin/compliance)"
echo ""

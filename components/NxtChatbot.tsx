...
function isAgentOnline(): boolean {
  const now = new Date();
  // Convert to EET (UTC+2 standard, UTC+3 summer — use UTC+2 as conservative)
  const eet = new Date(now.toLocaleString("en-US", { timeZone: "Europe/Vilnius" }));
  const day  = eet.getDay();   // 0=Sun, 6=Sat
  const hour = eet.getHours(); // 0-23
  return day >= 1 && day <= 5 && hour >= 9 && hour < 18;
}

function getAgentStatus() {
  const online = isAgentOnline();
  if (online) return { online: true,  text: "Agent Online",  color: "#0ECB81" };
  const now = new Date(new Date().toLocaleString("en-US", { timeZone: "Europe/Vilnius" }));
  const day  = now.getDay();
  const hour = now.getHours();
  if (day === 0 || day === 6) return { online: false, text: "Agent offline — opens Mon 9am EET", color: "#F0B90B" };
  if (hour < 9)  return { online: false, text: `Agent online at 9:00 EET`, color: "#F0B90B" };
...
export default function NxtChatbot() {
  ...
  // Pulse the button every few seconds to attract attention
  useEffect(() => {
  ...
  return (
    <>
      <style>{`
        .nxt-chat-widget{position:fixed;bottom:24px;right:24px;z-index:9998;font-family:'DM Sans',system-ui,sans-serif}
        
        /* TOGGLE BUTTON */
        .nxt-chat-btn{display:flex;align-items:center;gap:10px;padding:12px 20px;background:#0B0E11;border:1px solid rgba(240,185,11,0.35);border-radius:999px;cursor:pointer;box-shadow:0 4px 24px rgba(0,0,0,0.5);transition:all .2s;text-decoration:none}
        .nxt-chat-btn:hover{background:#161B22;border-color:rgba(240,185,11,0.6);transform:translateY(-1px)}
        .nxt-chat-btn-dot{width:8px;height:8px;border-radius:50%;background:#0ECB81;flex-shrink:0}
        .nxt-chat-btn-dot.pulse{animation:chatpulse 2s ease-in-out infinite}
        @keyframes chatpulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.8)}}
        .nxt-chat-btn-text{font-size:13px;font-weight:700;color:#fff;white-space:nowrap}
        .nxt-chat-btn-sub{font-size:10px;color:rgba(255,255,255,0.45);white-space:nowrap}
        .nxt-chat-btn-ico{font-size:18px;flex-shrink:0}
        .nxt-chat-btn-close{font-size:16px;color:rgba(255,255,255,0.5);font-weight:300}
        
        /* WINDOW */
        .nxt-chat-win{position:fixed;bottom:88px;right:24px;width:380px;max-width:calc(100vw - 32px);background:#0B0E11;border:1px solid rgba(255,255,255,0.1);border-radius:18px;overflow:hidden;box-shadow:0 16px 48px rgba(0,0,0,0.6);display:flex;flex-direction:column;animation:chatslide .2s ease;z-index:9997}
        ...
        .nxt-chat-tabs{display:flex;gap:0}
        ...
        .nxt-msg-time{font-size:10px;color:rgba(255,255,255,0.25);padding:0 4px}
        ...
        .nxt-chat-input{flex:1;background:#161B22;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:9px 12px;font-size:13px;color:#fff;outline:none;font-family:inherit;transition:border-color .15s;resize:none}
        ...
        @media(max-width:420px){.nxt-chat-win{width:calc(100vw - 16px);right:8px;bottom:80px}.nxt-chat-widget{right:8px;bottom:8px}}
      `}</style>
      ...
      <div className="nxt-chat-widget">
        {open ? (
          <>
            ...
            <button className="nxt-chat-btn" onClick={() => setOpen(false)}>
              <span className="nxt-chat-btn-ico">💬</span>
              <div>
                <div className="nxt-chat-btn-text">Support</div>
                <div className="nxt-chat-btn-sub">Click to close</div>
              </div>
              <span className="nxt-chat-btn-close">✕</span>
            </button>
          </>
        ) : (
          /* ── CLOSED BUTTON ── */
          <button className="nxt-chat-btn" onClick={() => setOpen(true)}>
            <span className={`nxt-chat-btn-dot ${pulse ? "pulse" : ""}`} />
            <span className="nxt-chat-btn-ico">💬</span>
            <div>
              <div className="nxt-chat-btn-text">24/7 AI Chat Support</div>
              <div className="nxt-chat-btn-sub">
                {agentStatus.online ? "🟢 Agent Online Now" : "⚫ Agent Offline · AI Available"}
              </div>
            </div>
          </button>
        )}

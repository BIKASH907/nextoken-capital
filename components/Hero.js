...
export default function Hero() {
  return (
    <section style={{background:"#0B0E11",minHeight:"92vh",display:"flex",alignItems:"center",padding:"100px 20px 60px",color:"#fff",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",width:"100%",position:"relative",zIndex:2,display:"grid",gridTemplateColumns:"1fr 380px",gap:60,alignItems:"center"}}>
        <div>
          ...
          <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:48}}>
            <Link href="/marketplace" style={{display:"inline-flex",alignItems:"center",gap:8,padding:"15px 36px",background:"#F0B90B",color:"#0B0E11",borderRadius:12,fontWeight:800,fontSize:15,textDecoration:"none",transition:"transform .15s",letterSpacing:"0.3px"}}>Browse Marketplace <span style={{fontSize:18}}>→</span></Link>
            <Link href="/tokenize" style={{display:"inline-flex",alignItems:"center",gap:8,padding:"15px 36px",background:"transparent",color:"#F0B90B",borderRadius:12,fontWeight:700,fontSize:15,textDecoration:"none",border:"1px solid rgba(240,185,11,0.25)",transition:"border-color .15s"}}>List Your Asset</Link>
          </div>
          <div style={{display:"flex",gap:36,paddingTop:28,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
            {[{n:"0.2%",l:"Marketplace fee"},{n:"EUR 100",l:"Minimum purchase"},{n:"ERC-3643",l:"Token standard"},{n:"Polygon",l:"Blockchain"}].map((s,i) => (
              <div key={i}>
        ...
        {/* Right side — Live asset card preview */}
        <div style={{background:"#0F1318",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:24,position:"relative"}}>
          ...
          <div style={{height:6,background:"rgba(255,255,255,0.04)",borderRadius:3,overflow:"hidden",marginBottom:8}}>
            <div style={{height:"100%",width:"34%",background:"linear-gradient(90deg, #F0B90B, #F0B90B)",borderRadius:3}} />
          </div>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"rgba(255,255,255,0.3)"}}>
            <span>34% funded</span>
            <span>EUR 340K / EUR 1M</span>
          </div>

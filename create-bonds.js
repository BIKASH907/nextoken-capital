...
const S = {
  ...
  btnGold: { padding:"12px 28px", borderRadius:10, background:"#d4af37", color:"#000", fontSize:14, fontWeight:700, border:"none", cursor:"pointer", textDecoration:"none", display:"inline-block", fontFamily:"inherit" },
  btnOut:  { padding:"12px 28px", borderRadius:10, background:"transparent", color:"#d4af37", fontSize:14, fontWeight:500, border:"1px solid rgba(212,175,55,0.35)", cursor:"pointer", textDecoration:"none", display:"inline-block", fontFamily:"inherit" },
  ...
  filterBtn: (a) => ({ padding:"6px 14px", borderRadius:20, border:"1px solid "+(a?"rgba(212,175,55,0.5)":"rgba(255,255,255,0.08)"), background:a?"rgba(212,175,55,0.12)":"transparent", color:a?"#d4af37":"#b0b0c8", fontSize:12.5, fontWeight:500, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" }),
  sortBtn:   (a) => ({ padding:"6px 12px", borderRadius:8, border:"1px solid "+(a?"rgba(212,175,55,0.5)":"rgba(255,255,255,0.08)"), background:a?"rgba(212,175,55,0.12)":"transparent", color:a?"#d4af37":"#7a7a96", fontSize:12, fontWeight:500, cursor:"pointer", fontFamily:"inherit" }),
};
...
const bonds = [
  { id:1, name:"SME Convertible Note I",    ticker:"SME-CNV-26",    type:"Convertible", issuer:"Growth Capital Partners", yield_:"8.2%", price:"99.8",  term:"2Y", rating:"BB",   progress:94, raised:"940K",  target:"1M",   min:"250",   status:"Closing", emoji:"💼", featured:true  },
  { id:2, name:"Logistics Income Bond",     ticker:"LOGI-28",       type:"Corporate",   issuer:"Baltic Logistics REIT",   yield_:"6.9%", price:"97.9",  term:"4Y", rating:"BBB",  progress:60, raised:"2.4M",  target:"4M",   min:"500",   status:"Live",    emoji:"🏭", featured:true  },
  { id:3, name:"Baltic Green Bond 2027",    ticker:"BALT-GREEN-27", type:"Green",       issuer:"Baltic Energy UAB",       yield_:"6.4%", price:"98.4",  term:"3Y", rating:"A-",   progress:72, raised:"3.6M",  target:"5M",   min:"500",   status:"Live",    emoji:"🌱", featured:true  },
  { id:4, name:"Renewable Yield Note 2030", ticker:"RYN-30",        type:"Green",       issuer:"Nord Renewables",         yield_:"5.8%", price:"100.4", term:"6Y", rating:"A",    progress:53, raised:"4.2M",  target:"8M",   min:"750",   status:"Live",    emoji:"💨", featured:false },
  { id:5, name:"EU Infrastructure Bond",   ticker:"EU-INFRA-29",   type:"Corporate",   issuer:"Euro Infra Group",        yield_:"5.1%", price:"101.2", term:"5Y", rating:"BBB+", progress:45, raised:"9M",    target:"20M",  min:"1000",  status:"Live",    emoji:"🏗", featured:false },
  { id:6, name:"Municipal Development Note",ticker:"MUNI-31",       type:"Municipal",   issuer:"Regional Dev Fund",       yield_:"4.3%", price:"102.1", term:"7Y", rating:"A+",   progress:65, raised:"6.5M",  target:"10M",  min:"1500",  status:"Upcoming",emoji:"🏛", featured:false },
];
...
export default function BondsPage() {
  ...
  const navLinks = [["Markets","/markets"],["Exchange","/exchange"],["Bonds","/bonds"],["Equity & IPO","/equity-ipo"],["Tokenize","/tokenize"]];
    ...
    <div style={S.page}>
      ...
      <div style={{ position:"relative", padding:"90px 32px 70px", textAlign:"center", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          <a href="#listings" style={S.btnGold}>Explore Bonds</a>
          <a href="#issue"    style={S.btnOut}>Issue a Bond</a>
        </div>
      </div>
      ...
      <div style={S.section}>
        ...
        <h2 style={S.h2}>Bond Categories You Can Access</h2>
      ...
      <div id="listings" style={S.section}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:10, alignItems:"center", marginBottom:28 }}>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {["All","Green","Corporate","Convertible","Municipal"].map((t) => (
              <button key={t} onClick={() => setActiveType(t)} style={S.filterBtn(activeType===t)}>{t}</button>
            ))}
          </div>
          <div style={{ width:1, height:28, background:"rgba(255,255,255,0.08)", margin:"0 4px" }} />
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {["All","Live","Closing","Upcoming"].map((s) => (
              <button key={s} onClick={() => setActiveStatus(s)} style={S.filterBtn(activeStatus===s)}>{s}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:20 }}>
          ...
          {filtered.length===0 && (
            <div style={{ gridColumn:"1/-1", padding:"60px 0", textAlign:"center", color:"#7a7a96" }}>No bonds match this filter.</div>
          )}
        </div>
      </div>
            ...
            <tbody>
              {bonds.map((b, i) => (
                <tr key={i} style={{ borderBottom:"1px solid rgba(255,255,255,0.06)" }}
                  ...
                  <td style={{ padding:"14px 16px" }}>
                    <span style={{ padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:600, background:(typeColor[b.type]||typeColor.Corporate).bg, color:(typeColor[b.type]||typeColor.Corporate).color, border:"1px solid "+(typeColor[b.type]||typeColor.Corporate).border }}>{b.type}</span>
                  </td>
                  <td style={{ padding:"14px 16px", fontSize:12.5, color:"#b0b0c8" }}>{b.issuer}</td>
                  <td style={{ padding:"14px 16px", fontFamily:"Syne,sans-serif", fontWeight:700, color:"#22c55e", fontSize:14 }}>{b.yield_}</td>
                  ...
                  <td style={{ padding:"14px 16px" }}>
                    <span style={{ padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:600, background:(statusStyle[b.status]||statusStyle.Live).bg, color:(statusStyle[b.status]||statusStyle.Live).color, border:"1px solid "+(statusStyle[b.status]||statusStyle.Live).border }}>{b.status}</span>
                  </td>
      ...
      <div style={{ margin:"0 32px 64px", borderRadius:18, padding:"64px 48px", textAlign:"center", position:"relative", overflow:"hidden", border:"1px solid rgba(212,175,55,0.3)", background:"linear-gradient(135deg,rgba(212,175,55,0.10) 0%,rgba(14,165,233,0.05) 100%)" }}>
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          <span style={S.label}>Ready to Launch?</span>
          ...
          <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
            <a href="/tokenize" style={S.btnGold}>Issue a Bond</a>
            <a href="/exchange" style={S.btnOut}>Explore Exchange</a>
          </div>
          <p style={{ fontSize:11, color:"#7a7a96", marginTop:24, opacity:0.7 }}>Bond market notice: Yield, price, maturity, and secondary market access may vary depending on issuer structure and jurisdiction.</p>
        </div>
      </div>
...
console.log("Done! app/bonds/page.tsx created - " + code.length + " characters");
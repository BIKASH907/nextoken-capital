...
export default async function handler(req, res) {
  ...
  if (authError) {
    return res.redirect("/tokenize?monerium=error&reason=" + encodeURIComponent(authError));
  }
  if (!code) {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return res.redirect("/tokenize?monerium=error&reason=token_exchange_failed");
    }
    ...
    // Redirect back to tokenize page (or issuer dashboard)

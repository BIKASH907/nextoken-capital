...
export default function IssuerDocuments() {
  ...
  useEffect(() => {
    if (mounted && session) {
      fetch("/api/assets/my-listings").then(r => r.json()).then(d => {
        setAssets(d.assets || []);
        if (d.assets && d.assets[0]) setSelectedAsset(d.assets[0]._id);
        setLoading(false);
    ...
    try {
      ...
      const res = await fetch("/api/assets/documents", {
        ...
        body: JSON.stringify({
          ...
          fileName: uploadForm.file.name, fileUrl: ud.url || ud.fileUrl || "",
  ...
  const deleteDoc = async (id) => {
    ...
    await fetch("/api/assets/documents", {
  ...
  const badge = (s) => {
    const c = { pending: "#f59e0b", approved: "#22c55e", rejected: "#ef4444" };
    return <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: (c[s] || "#666") + "15", color: c[s] || "#666", fontWeight: 700 }}>{s}</span>;
  };

  const visBadge = (v) => {
    const c = { public: "#22c55e", investors_only: "#3b82f6", admin_only: "#f59e0b" };
    const l = { public: "Public", investors_only: "Investors", admin_only: "Admin" };
    return <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 3, background: (c[v] || "#666") + "15", color: c[v] || "#666", fontWeight: 600 }}>{l[v] || v}</span>;
  };
  ...
  if (!session) {
    return (
      <div style={{ minHeight: "100vh", background: "#0B0E11", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        <div style={{ textAlign: "center" }}>
          <p>Please login first</p>
          <button onClick={() => router.push("/login")} style={{ padding: "10px 24px", background: "#F0B90B", color: "#000", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer", marginTop: 10 }}>Login</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Head><title>Asset Documents — Nextoken</title></Head>
      <Navbar />
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>Upload legal, financial, and operational documents</p>
            </div>
          ...
          {msg && (
            <div style={{ background: msg.startsWith("Error") ? "rgba(255,77,77,0.1)" : "rgba(34,197,94,0.1)", border: "1px solid " + (msg.startsWith("Error") ? "rgba(255,77,77,0.2)" : "rgba(34,197,94,0.2)"), borderRadius: 8, padding: "10px 14px", fontSize: 13, color: msg.startsWith("Error") ? "#ff6b6b" : "#22c55e", marginBottom: 16 }}>{msg}</div>
          )}
          ...
          {selectedAsset && (
            <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 20 }}>
              <div>
                {Object.entries(CATEGORIES).map(([k, ci]) => (
                  <div key={k} onClick={() => setCat(k)} style={{ padding: "12px 16px", borderRadius: 8, marginBottom: 4, cursor: "pointer", background: cat === k ? "#F0B90B15" : "rgba(255,255,255,0.03)", border: cat === k ? "1px solid #F0B90B30" : "1px solid rgba(255,255,255,0.05)" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: cat === k ? "#F0B90B" : "rgba(255,255,255,0.6)" }}>{ci.icon} {ci.label}</div>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>{ci.desc}</div>
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                <div style={{ background: "#161b22", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
                  {docs.length === 0 ? (
                    <div style={{ padding: 30, textAlign: "center", color: "rgba(255,255,255,0.3)" }}>No documents uploaded yet</div>
                  ) : docs.map((d, i) => (
                    <div key={i} style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        <a href={d.fileUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#F0B90B", textDecoration: "none", fontWeight: 600 }}>{d.fileName}</a>
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>{d.docType}</div>
                      </div>

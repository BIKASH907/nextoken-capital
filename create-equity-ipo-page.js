...
const listings = [
  { id:1, emoji:"⚡", name:"VoltGrid Energy",      ticker:"VGE", subtype:"Blockchain IPO", type:"ipo",       risk:"low",    location:"Helsinki, Finland",      irr:"28.4%", min:"EUR 100",   target:"EUR 18M", raised:"EUR 15.7M", pct:87, status:"hot",      sl:"Hot"         },
  { id:2, emoji:"🤖", name:"NeuroLogic AI",         ticker:"NLA", subtype:"Series A",      type:"early",     risk:"medium", location:"Tallinn, Estonia",       irr:"34.2%", min:"EUR 500",   target:"EUR 25M", raised:"EUR 15.3M", pct:61, status:"live",     sl:"Live"        },
  { id:3, emoji:"🏥", name:"MedCore Pharma",         ticker:"MCP", subtype:"ERC-3643",      type:"token",     risk:"low",    location:"Zurich, Switzerland",    irr:"19.8%", min:"EUR 250",   target:"EUR 40M", raised:"EUR 29.6M", pct:74, status:"live",     sl:"Live"        },
  { id:4, emoji:"🌊", name:"AquaTech Solutions",     ticker:"ATS", subtype:"Blockchain IPO", type:"ipo",      risk:"medium", location:"Amsterdam, Netherlands", irr:"22.1%", min:"EUR 500",   target:"EUR 12M", raised:"EUR 11.2M", pct:93, status:"closing",  sl:"Closing Soon"},
  { id:5, emoji:"🛸", name:"OrbitalX Space",         ticker:"ORB", subtype:"Seed Round",    type:"early",     risk:"high",   location:"Warsaw, Poland",         irr:"31.7%", min:"EUR 1,000", target:"EUR 8M",  raised:"EUR 3.0M",  pct:38, status:"live",     sl:"Live"        },
  { id:6, emoji:"📦", name:"Baltic Logistics REIT",  ticker:"BLR", subtype:"Secondary",     type:"secondary", risk:"low",    location:"Riga, Latvia",           irr:"16.3%", min:"EUR 250",   target:"EUR 6M",  raised:"EUR 1.3M",  pct:22, status:"upcoming", sl:"Upcoming"    },
];
    ...
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, padding:14, background:"rgba(255,255,255,0.025)", borderRadius:10, marginBottom:16 }}>
        {[{v:item.irr,l:"Target IRR"},{v:item.min,l:"Min. Invest"},{v:item.target,l:"Raise Target"}].map((m) => (
          <div key={m.l}>
            <div style={{ fontFamily:"Syne,sans-serif", fontSize:17, fontWeight:700, color:"#F0B90B" }}>{m.v}</div>
            <div style={{ fontSize:10.5, color:"#7a7a96", marginTop:2 }}>{m.l}</div>
          </div>
        ))}
      </div>
      <div style={{ marginBottom:14 }}>
        <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#7a7a96", marginBottom:6 }}>
          <span>Funding {item.pct}%</span>
...
const FB = (active) => ({ padding:"6px 14px", borderRadius:20, border:"1px solid "+(active?"rgba(240,185,11,0.5)":"rgba(255,255,255,0.08)"), background:active?"rgba(240,185,11,0.12)":"transparent", color:active?"#F0B90B":"#b0b0c8", fontSize:12.5, fontWeight:500, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" });
    ...
    <div style={S.page}>
      ...
      <div style={{ position:"relative", padding:"90px 32px 70px", textAlign:"center", overflow:"hidden" }}>
        ...
        <h1 style={{ fontFamily:"Syne,sans-serif", fontSize:"clamp(36px,6vw,68px)", fontWeight:800, lineHeight:1.05, letterSpacing:"-1.5px", color:"#e8e8f0", maxWidth:820, margin:"0 auto 20px" }}>
          Digital Equity and<br /><span style={{ color:"#F0B90B" }}>Blockchain IPO Market</span>
        </h1>
        ...
        <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          <a href="#listings" style={S.gold}>Explore Listings</a>
          <a href="#issue"    style={S.out}>Issue Equity</a>
        </div>
      </div>
      ...
      <div id="listings" style={S.sec}>
        ...
        <h2 style={S.h2}>Equity and IPO Directory</h2>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:10, alignItems:"center", marginBottom:28 }}>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {typeFilters.map(([v,l]) => <button key={v} onClick={() => setActiveType(v)} style={FB(activeType===v)}>{l}</button>)}
          </div>
          <div style={{ width:1, height:28, background:"rgba(255,255,255,0.08)", margin:"0 4px" }} />
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {riskFilters.map(([v,l]) => <button key={v} onClick={() => setActiveRisk(v)} style={FB(activeRisk===v)}>{l}</button>)}
          </div>
        </div>
      ...
      <div style={S.sec}>
        ...
        <div style={{ overflowX:"auto", border:"1px solid rgba(255,255,255,0.08)", borderRadius:14 }}>
          <table>
            ...
            <tbody>
              {[
                { name:"NeuroLogic AI",         ticker:"NLA", type:"Early-Stage",    stage:"Series A",    irr:"34.2%", price:"EUR 1.42", cap:"EUR 52M",  risk:"medium", prog:"61%", min:"EUR 500",   status:"live"     },
                { name:"OrbitalX Space",        ticker:"ORB", type:"Early-Stage",    stage:"Seed",        irr:"31.7%", price:"EUR 0.88", cap:"EUR 14M",  risk:"high",   prog:"38%", min:"EUR 1,000", status:"live"     },
                { name:"VoltGrid Energy",       ticker:"VGE", type:"Blockchain IPO", stage:"Public",      irr:"28.4%", price:"EUR 2.10", cap:"EUR 80M",  risk:"low",    prog:"87%", min:"EUR 100",   status:"hot"      },
                { name:"AquaTech Solutions",    ticker:"ATS", type:"Blockchain IPO", stage:"Public",      irr:"22.1%", price:"EUR 1.55", cap:"EUR 35M",  risk:"medium", prog:"93%", min:"EUR 500",   status:"closing"  },
                { name:"MedCore Pharma",        ticker:"MCP", type:"Equity Token",   stage:"ERC-3643",    irr:"19.8%", price:"EUR 3.20", cap:"EUR 190M", risk:"low",    prog:"74%", min:"EUR 250",   status:"live"     },
                { name:"Baltic Logistics REIT", ticker:"BLR", type:"Secondary",      stage:"Token Float", irr:"16.3%", price:"EUR 1.08", cap:"EUR 28M",  risk:"low",    prog:"22%", min:"EUR 250",   status:"upcoming" },
              ].map((row, i) => (
                <tr key={i} style={{ borderBottom:"1px solid rgba(255,255,255,0.06)" }}
                  ...
                  <td style={{ padding:"14px 16px" }}>
                    <span style={{ ...(statusStyle[row.status]||statusStyle.live), padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:600 }}>
                      {row.status==="live"?"Live":row.status==="hot"?"Hot":row.status==="closing"?"Closing":"Upcoming"}
                    </span>
                  </td>
      ...
      <div id="issue" style={{ background:"#0a0a10", borderTop:"1px solid rgba(255,255,255,0.06)", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
        <div style={S.sec}>
          ...
          <div style={{ position:"relative", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:32 }}>
            ...
            {[
              { n:"01", t:"Define Structure",   b:"Set share class, supply, valuation cap, investor rights, and preferred terms." },
              { n:"02", t:"Submit Legal Docs",   b:"Prepare cap table, pitch deck, audited financials, and disclosure package." },
              { n:"03", t:"Token Issuance",      b:"ERC-3643 security tokens with KYC-gated investor whitelisting on-chain." },
              { n:"04", t:"Run the Fundraise",   b:"Open subscriptions, monitor allocation progress and milestones in real time." },
              { n:"05", t:"Exchange Listing",    b:"Qualified tokens progress to Nextoken secondary exchange for liquidity." },
            ].map((s) => (
              <div key={s.n}>
...
fs.writeFileSync("pages/equity-ipo.js", code, "utf8");
console.log("Done! pages/equity-ipo.js — " + code.length + " chars — plain JS, no TypeScript");
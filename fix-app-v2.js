...
function WalletBackupReminder() {
  ...
  useEffect(() => {
    try { if (!localStorage.getItem("nxt_wallet_backup")) setTimeout(() => setShow(true), 5000); } catch(e) {}
  }, []);
  function dismiss() {
...
function CookieBanner() {
  const [show, setShow] = useState(false);
  useEffect(() => {
    try { if (!localStorage.getItem("nxt_cookies")) setTimeout(() => setShow(true), 2000); } catch(e) {}
  }, []);
  const accept  = () => { try { localStorage.setItem("nxt_cookies","accepted");  } catch(e) {} setShow(false); };
  const decline = () => { try { localStorage.setItem("nxt_cookies","declined"); } catch(e) {} setShow(false); };
  if (!show) return null;
  return (
    <div style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:9998, background:"rgba(13,13,20,0.98)", borderTop:"1px solid rgba(240,185,11,0.2)", padding:"16px 32px", backdropFilter:"blur(20px)", display:"flex", flexWrap:"wrap", alignItems:"center", gap:16, justifyContent:"space-between" }}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        <button onClick={decline} style={{ padding:"9px 20px", borderRadius:8, border:"1px solid rgba(255,255,255,0.12)", background:"transparent", color:"#8a9bb8", fontSize:13, cursor:"pointer" }}>Decline</button>
        <button onClick={accept}  style={{ padding:"9px 20px", borderRadius:8, background:"#F0B90B", color:"#000", fontSize:13, fontWeight:800, border:"none", cursor:"pointer" }}>Accept All</button>
      </div>
    </div>
  );
}

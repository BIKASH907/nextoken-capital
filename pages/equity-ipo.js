...
const statusStyle = {
  live:     { background:"rgba(34,197,94,0.10)",  color:"#0ECB81", border:"1px solid rgba(34,197,94,0.25)"  },
  hot:      { background:"rgba(239,68,68,0.10)",  color:"#ef4444", border:"1px solid rgba(239,68,68,0.25)"  },
  closing:  { background:"rgba(212,175,55,0.10)", color:"#F0B90B", border:"1px solid rgba(212,175,55,0.3)"  },
  upcoming: { background:"rgba(99,102,241,0.10)", color:"#8b5cf6", border:"1px solid rgba(99,102,241,0.3)"  },
};
      ...
      <div style={{ background:"#0B0E11",border:"1px solid rgba(240,185,11,0.25)",borderRadius:18,padding:28,width:"100%",maxWidth:420,position:"relative" }}>
        ...
        {step==="success"?(
          ...
          <>
            ...
            <div style={{fontFamily:"Syne,sans-serif",fontSize:17,fontWeight:800,color:"rgba(255,255,255,0.85)",marginBottom:2}}>{item.name}</div>
            <div style={{fontFamily:"monospace",fontSize:11,color:"rgba(255,255,255,0.45)",marginBottom:16}}>{item.ticker} · {item.subtype}</div>
...
function ListingCard({ item, onInvest }) {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ background:"#0B0E11",border:"1px solid "+(hov?"rgba(240,185,11,0.35)":"rgba(255,255,255,0.08)"),borderRadius:16,padding:24,transition:"all 0.2s",position:"relative",overflow:"hidden",transform:hov?"translateY(-2px)":"none" }}>
      ...
      <div style={{fontFamily:"Syne,sans-serif",fontSize:17,fontWeight:700,color:"rgba(255,255,255,0.85)",marginBottom:2}}>{item.name}</div>
      <div style={{fontFamily:"monospace",fontSize:11,color:"rgba(255,255,255,0.45)",marginBottom:4}}>{item.ticker}</div>
      <div style={{fontSize:12.5,color:"rgba(255,255,255,0.45)",marginBottom:16}}>Location: {item.location}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,padding:14,background:"rgba(255,255,255,0.025)",borderRadius:10,marginBottom:16}}>
        {[{v:item.irr,l:"Target IRR"},{v:item.min,l:"Min. Invest"},{v:item.target,l:"Raise Target"}].map(m=>(
          <div key={m.l}><div style={{fontFamily:"Syne,sans-serif",fontSize:17,fontWeight:700,color:"#F0B90B"}}>{m.v}</div><div style={{fontSize:10.5,color:"rgba(255,255,255,0.45)",marginTop:2}}>{m.l}</div></div>
        ))}
      </div>
      <div style={{marginBottom:14}}>
        ...
        <div style={{height:5,background:"rgba(255,255,255,0.07)",borderRadius:4,overflow:"hidden"}}>
          <div style={{width:item.pct+"%",height:"100%",background:"linear-gradient(90deg,#F0B90B,#F0B90B)",borderRadius:4}}/>
        </div>
      </div>
              ...
              <tbody>
                {tableRows.map((row,i)=>(
                  <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.06)"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.02)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    <td style={{padding:"14px 16px"}}><div style={{fontWeight:600,fontSize:13.5,color:"rgba(255,255,255,0.85)"}}>{row.name}</div><div style={{fontFamily:"monospace",fontSize:10.5,color:"rgba(255,255,255,0.45)"}}>{row.ticker}</div></td>
                    ...
                    <td style={{padding:"14px 16px",fontSize:13,color:"rgba(255,255,255,0.6)"}}>{row.stage}</td>
                    <td style={{padding:"14px 16px",fontFamily:"Syne,sans-serif",fontWeight:700,color:"#0ECB81"}}>{row.irr}</td>
                    <td style={{padding:"14px 16px",fontFamily:"monospace",fontSize:12.5,color:"rgba(255,255,255,0.85)"}}>{row.price}</td>
                    <td style={{padding:"14px 16px",fontFamily:"monospace",fontSize:12.5,color:"rgba(255,255,255,0.6)"}}>{row.cap}</td>
                    ...
                    <td style={{padding:"14px 16px",fontSize:12.5,color:"rgba(255,255,255,0.6)"}}>{row.prog}</td>
                    <td style={{padding:"14px 16px",fontFamily:"monospace",fontSize:12.5,color:"rgba(255,255,255,0.85)"}}>{row.min}</td>

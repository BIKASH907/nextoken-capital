 ...
 *  1.  Utilities & Helpers
 *  2.  State Management
 *  3.  Router / Page System
 *  4.  Market Data Engine (Live Simulation)
 *  5.  Ticker Bar
 *  6.  Price Chart (Canvas)
 *  7.  Order Book
 *  8.  Trade History Feed
 *  9.  Exchange / Trading Logic
 * 10.  Toast Notification System
 * 11.  Modal System
 * 12.  Auth Forms (Login / Register)
 * 13.  KYC Status Widget
 * 14.  API Key Manager
 * 15.  API Docs — Interactive Playground
 * 16.  Blog Category Filter & Search
 * 17.  Careers — Job Filter & Apply Modal
 * 18.  Scroll Animations & Counters
 * 19.  Particle / Grid Background
...
NXT.utils = {
  ...
  /** Safely query selector with error-free fallback */
...
NXT.router = {
  ...
  init() {
    ...
    window.addEventListener('hashchange', () => {
      ...
      if (this.pages.includes(hash)) this.go(hash, false);
    });
...
NXT.market = {
  ...
  priceHistory: {},      // { 'BTC/EUR': [{ time, open, high, low, close, vol }] }
  _timers: [],
  _subscribers: [],

  init() {
    // Build initial candle history (200 candles)
    Object.keys(this.assets).forEach(pair => {
      this.priceHistory[pair] = this._generateHistory(pair, 200);
    });

    // Live price simulation — each asset ticks independently
    Object.entries(this.assets).forEach(([pair, data]) => {
      const interval = NXT.utils.randInt(800, 2200);
      const timer = setInterval(() => this._tick(pair), interval);
      this._timers.push(timer);
    });

    // New candle every 30s
    const candleTimer = setInterval(() => this._closeCandle(), 30000);
    this._timers.push(candleTimer);
  },

  _generateHistory(pair, count) {
    const asset   = this.assets[pair];
    let price     = asset.price * NXT.utils.rand(0.85, 1.15);
    const candles = [];
    const now     = Date.now();
    ...
    for (let i = count; i > 0; i--) {
      ...
      candles.push({ time: now - i * 60000, open, high, low, close, vol });
      price = close;
    }
    return candles;
  },
  ...
  _tick(pair) {
    const asset      = this.assets[pair];
    const volatility = asset.price * 0.0008;
    const delta      = NXT.utils.rand(-volatility, volatility);
    const newPrice   = Math.max(asset.price + delta, 0.0001);

    asset.prev  = asset.price;
    asset.price = parseFloat(newPrice.toFixed(pair.includes('XRP') || pair.includes('ADA') || pair.includes('MATIC') ? 4 : 2));
    asset.change = ((asset.price - asset.open) / asset.open) * 100;

    // Update last candle high/low
    ...
    this._subscribers.forEach(fn => fn(pair, asset));
    NXT.state.emit('price:update', { pair, asset });
  },
  ...
  _closeCandle() {
    Object.entries(this.assets).forEach(([pair, asset]) => {
      ...
      history.push({
        time:  Date.now(),
        open:  last ? last.close : asset.price,
        high:  asset.price,
        low:   asset.price,
        close: asset.price,
        vol:   NXT.utils.rand(asset.vol * 0.4, asset.vol * 1.6),
      });
...
NXT.ticker = {
  ...
  render() {
    const pairs = Object.keys(NXT.market.assets);
    const items = pairs.map(pair => this._buildItem(pair)).join('');
    // Duplicate for seamless loop
    this.track.innerHTML = items + items;
  },

  _buildItem(pair) {
    const asset  = NXT.market.assets[pair];
    const change = ((asset.price - asset.open) / asset.open) * 100;
    const sign   = change >= 0 ? '+' : '';
    const cls    = change >= 0 ? 'up' : 'down';

    return `
      <span class="ticker-item" data-pair="${pair}">
        <span class="ticker-pair">${pair}</span>
        <span class="ticker-price" data-price="${pair}">
          ${asset.price.toLocaleString('en-EU', { minimumFractionDigits: 2 })}
        </span>
        <span class="ticker-change ${cls}" data-change="${pair}">
  ...
  _updateCell(pair, asset) {
    ...
    NXT.utils.$$(`[data-price="${pair}"]`).forEach(el => {
      el.textContent = asset.price.toLocaleString('en-EU', { minimumFractionDigits: 2 });
      el.classList.add('flash');
      setTimeout(() => el.classList.remove('flash'), 400);
    });

    NXT.utils.$$(`[data-change="${pair}"]`).forEach(el => {
...
NXT.chart = {
  ...
  init(canvasId = 'price-chart') {
    ...
    // Subscribe to price updates
    NXT.market.subscribe((pair) => {
      if (pair === this.pair) { this._dirty = true; }
    });
  ...
  _draw() {
    ...
    const data   = NXT.market.priceHistory[this.pair] || [];
    const VIEW   = Math.min(data.length, 80);
    const candles = data.slice(-VIEW);
    ...
    const TEXT = 'rgba(122,133,153,0.9)';
    ...
    bg.addColorStop(0, 'rgba(13,17,32,0)');
    bg.addColorStop(1, 'rgba(13,17,32,0)');
    ctx.fillStyle = bg;
    ...
    if (this.type === 'candle') {
      this._drawCandles(ctx, candles, toX, toY, chartW, UP, DOWN);
    } else {
      this._drawLine(ctx, candles, toX, toY, chartW, chartH, PAD, UP, DOWN);
    }
    ...
    // ── Hover tooltip
    if (this._hoverIdx >= 0 && this._hoverIdx < candles.length) {
      this._drawTooltip(ctx, candles[this._hoverIdx], toX(this._hoverIdx), PAD.top, H, PAD.bottom);
    }
    ...
    if (lastPrice) {
      ...
      ctx.save();
      ctx.setLineDash([4, 4]);
      ctx.strokeStyle = 'rgba(56,189,130,0.4)';
      ctx.lineWidth   = 1;
      ctx.beginPath(); ctx.moveTo(PAD.left, y); ctx.lineTo(W - PAD.right, y); ctx.stroke();
      ctx.restore();
      ...
      ctx.fillStyle = UP;
      ctx.beginPath();
      ctx.roundRect(W - PAD.right + 2, y - 9, 55, 18, 4);
      ctx.fill();
  ...
  _drawCandles(ctx, candles, toX, toY, chartW, UP, DOWN) {
    ...
    candles.forEach((c, i) => {
      ...
      ctx.fillStyle   = c.close >= c.open ? UP : 'rgba(224,80,80,0.85)';
      ctx.lineWidth   = 1;
      ...
      const ht  = Math.max(1, Math.abs(toY(c.open) - toY(c.close)));
      ctx.fillRect(x - w / 2, top, w, ht);

      // Hover highlight
      if (i === this._hoverIdx) {
        ctx.fillStyle = 'rgba(255,255,255,0.06)';
        ctx.fillRect(x - w, 0, w * 2, toY(0) + 100);
      }
    });
  },
  ...
  _drawLine(ctx, candles, toX, toY, chartW, chartH, PAD, UP, DOWN) {
    ...
    grad.addColorStop(0, isUp ? 'rgba(56,189,130,0.18)' : 'rgba(224,80,80,0.18)');
    ...
    ctx.beginPath();
    candles.forEach((c, i) => {
      if (i === 0) ctx.moveTo(toX(i), toY(c.close));
      else         ctx.lineTo(toX(i), toY(c.close));
    });
    ctx.strokeStyle = color;
    ctx.lineWidth   = 1.5;
    ctx.lineJoin    = 'round';
  ...
  _drawVolume(ctx, candles, toX, W, H, PAD, chartW, UP, DOWN) {
    ...
    const volH   = (H - PAD.bottom - PAD.top) * 0.14;
    const barW   = Math.max(1, (chartW / candles.length) * 0.6);

    candles.forEach((c, i) => {
      const x    = toX(i);
      const barHeight = (c.vol / maxVol) * volH;
  ...
  _drawTooltip(ctx, candle, x, topPad, H, bottomPad) {
    ...
    const lines = [
      ...
      `V: ${NXT.utils.formatCompact(candle.vol)}`,
    ];
    const TW = 120, TH = lines.length * 16 + 16;
    let tx = x + 12;
    if (tx + TW > ctx.canvas.clientWidth - 60) tx = x - TW - 12;

    ctx.fillStyle = 'rgba(13,17,32,0.92)';
    ctx.strokeStyle = 'rgba(56,189,130,0.3)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.roundRect(tx, topPad + 10, TW, TH, 6);
    ctx.fill(); ctx.stroke();

    ctx.font = '10px DM Mono, monospace';
    lines.forEach((line, i) => {
      ctx.fillStyle = i === 0 ? 'rgba(255,255,255,0.7)' : 'rgba(122,133,153,0.9)';
      ctx.textAlign = 'left';
      ctx.fillText(line, tx + 8, topPad + 24 + i * 16);
    });
  ...
  _onMouseMove(e) {
    ...
    const idx = Math.floor((mouseX - PAD_L) / (chartW / candles.length));
    this._hoverIdx = NXT.utils.clamp(idx, 0, candles.length - 1);
    this._dirty    = true;
  },
...
NXT.orderBook = {
  ...
  init(containerId = 'order-book') {
    ...
    NXT.market.subscribe((pair) => {
      if (pair === this.pair) { this._generate(); this.render(); }
    });
  },
  ...
  _generate() {
    const asset = NXT.market.assets[this.pair];
    const mid   = asset?.price || 62000;
    const spread = mid * 0.0003;

    this._bids = [];
    this._asks = [];

    let bidP = mid - spread / 2;
    let askP = mid + spread / 2;

    for (let i = 0; i < this.DEPTH; i++) {
      const vol    = NXT.utils.rand(0.05, 3.5);
      bidP -= NXT.utils.rand(0.1, mid * 0.0005);
      this._bids.push({ price: bidP, vol, total: 0 });
    }
    for (let i = 0; i < this.DEPTH; i++) {
      const vol = NXT.utils.rand(0.05, 3.5);
      askP += NXT.utils.rand(0.1, mid * 0.0005);
      this._asks.push({ price: askP, vol, total: 0 });
    }

    // Cumulative totals
    let cumBid = 0, cumAsk = 0;
    this._bids.forEach(b => { cumBid += b.vol; b.total = cumBid; });
    this._asks.forEach(a => { cumAsk += a.vol; a.total = cumAsk; });

    this._maxTotal = Math.max(cumBid, cumAsk);
  },

  _mutate() {
    const mutIdx   = NXT.utils.randInt(0, this.DEPTH - 1);
    const mutSide  = Math.random() > 0.5 ? this._bids : this._asks;
    if (mutSide[mutIdx]) {
      mutSide[mutIdx].vol = NXT.utils.rand(0.02, 4);
    }
    let cb = 0, ca = 0;
    this._bids.forEach(b => { cb += b.vol; b.total = cb; });
    this._asks.forEach(a => { ca += a.vol; a.total = ca; });
  ...
  render() {
    ...
    const asset  = NXT.market.assets[this.pair];
    const mid    = asset?.price || 62000;
    const change = ((asset?.price - asset?.open) / asset?.open * 100) || 0;
    const cls    = change >= 0 ? 'up' : 'down';

    const rows = (entries, side) => entries.map(e => {
      const pct  = (e.total / this._maxTotal) * 100;
      const cls2 = side === 'bid' ? 'ob-bid' : 'ob-ask';
      return `
        <div class="ob-row ${cls2}" style="position:relative;">
          <div class="ob-depth-bar ${cls2}-bar" style="width:${pct.toFixed(1)}%"></div>
          <span class="ob-price">${e.price.toFixed(2)}</span>
          <span class="ob-vol">${e.vol.toFixed(4)}</span>
          <span class="ob-total">${e.total.toFixed(4)}</span>
        </div>`;
    }).join('');
    ...
    this.el.innerHTML = `
      ...
      <div class="ob-asks">${rows([...this._asks].reverse(), 'ask')}</div>
      <div class="ob-mid ${cls}">${mid.toLocaleString('en-EU', { minimumFractionDigits: 2 })} <small>${change >= 0 ? '+' : ''}${change.toFixed(2)}%</small></div>
      <div class="ob-bids">${rows(this._bids, 'bid')}</div>`;
  },
...
NXT.toast.show('🎉 Application submitted! We\'ll be in touch within 5 business days.', 'success', 7000);
    ...
    // New trade every 1–3s
    const schedule = () => {
      this._interval = setTimeout(() => {
        this._generateTrade(false);
        this.render();
        schedule();
      }, NXT.utils.randInt(600, 2800));
    };
    schedule();
  },
  ...
  _generateTrade(silent = false) {
    const asset  = NXT.market.assets[this.pair];
    const price  = asset.price * NXT.utils.rand(0.9995, 1.0005);
    const amount = NXT.utils.rand(0.001, 2.5);
    ...
    if (this.trades.length > this._MAX) this.trades.pop();
  },
  ...
  render() {
    ...
    this.el.innerHTML = `
      ...
      ${this.trades.map((t, i) => `
        <div class="tf-row ${t.side} ${i === 0 && t.isNew ? 'tf-new' : ''}">
          <span class="tf-price ${t.side}">${t.price.toLocaleString('en-EU', { minimumFractionDigits: 2 })}</span>
          <span class="tf-amount">${t.amount.toFixed(4)}</span>
          <span class="tf-time">${new Date(t.time).toLocaleTimeString('en-EU', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
        </div>`).join('')}`;
  },
...
NXT.exchange = {
  ...
  init() {
    ...
    // Side tabs
    NXT.utils.$$('[data-side]').forEach(btn => {
      btn.addEventListener('click', () => {
        this.side = btn.dataset.side;
        NXT.utils.$$('[data-side]').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        this._updateFormColor();
      });
    });

    // Order type tabs
    NXT.utils.$$('[data-order-type]').forEach(btn => {
      btn.addEventListener('click', () => {
        this.type = btn.dataset.orderType;
        NXT.utils.$$('[data-order-type]').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        this._togglePriceField();
      });
    });

    // Percentage quick-fill
    NXT.utils.$$('[data-pct]').forEach(btn => {
      btn.addEventListener('click', () => {
        const pct    = parseFloat(btn.dataset.pct) / 100;
        const balance = this.side === 'buy' ? 5000 : 0.15;
        const priceEl = document.getElementById('order-price');
        const amtEl   = document.getElementById('order-amount');
        if (amtEl) amtEl.value = (balance * pct).toFixed(this.side === 'buy' ? 2 : 6);
        this._updateTotal();
      });
    });
    ...
    ['order-amount', 'order-price'].forEach(id => {
      ...
      if (el) el.addEventListener('input', () => this._updateTotal());
    });
    ...
    // Fill current price
    NXT.market.subscribe((pair) => {
      if (pair === NXT.state.activeMarket) this._fillCurrentPrice();
    });
  ...
  _updateFormColor() {
    ...
    submitBtn.textContent = this.side === 'buy'
      ? `Buy ${NXT.state.activeMarket.split('/')[0]}`
      : `Sell ${NXT.state.activeMarket.split('/')[0]}`;
  },

  _togglePriceField() {
    const priceRow = document.getElementById('price-row');
    if (priceRow) {
      priceRow.style.display = this.type === 'market' ? 'none' : '';
    }
  },

  _fillCurrentPrice() {
    const priceEl = document.getElementById('order-price');
    if (!priceEl || this.type === 'market') return;
    const asset = NXT.market.assets[NXT.state.activeMarket];
    if (asset) priceEl.value = asset.price.toFixed(2);
    this._updateTotal();
  },
  ...
  _updateTotal() {
    ...
    totalEl.textContent = NXT.utils.formatCurrency(total);
  },

  _placeOrder() {
    if (!NXT.state.isLoggedIn) {
      NXT.modal.open('login');
      NXT.toast.show('Please log in to place orders', 'warn');
      return;
    }
    if (NXT.state.kycLevel < 2) {
      NXT.toast.show('KYC Level 2 required for trading', 'warn');
      return;
    }
    ...
    if (!amount || amount <= 0) {
      NXT.toast.show('Please enter a valid amount', 'error');
      return;
    }

    const price  = this.type === 'market'
      ? NXT.market.assets[NXT.state.activeMarket]?.price
      : parseFloat(priceEl?.value);

    const order = {
      id:      NXT.utils.uid('ord'),
      pair:    NXT.state.activeMarket,
      side:    this.side,
      type:    this.type,
      amount,
      price,
    ...
    const base = NXT.state.activeMarket.split('/')[0];
    NXT.toast.show(
      `${this.side === 'buy' ? '🟢' : '🔴'} ${this.side.toUpperCase()} order placed — ${amount} ${base} @ ${NXT.utils.formatCurrency(price)}`,
      'success'
    );
    ...
    setTimeout(() => {
      order.status = 'filled';
      NXT.state.openOrders = NXT.state.openOrders.filter(o => o.id !== order.id);
      NXT.toast.show(`✅ Order ${order.id} filled`, 'success');
      NXT.portfolio.refresh();
    }, NXT.utils.randInt(1500, 5000));
...
NXT.toast = {
  ...
  init() {
    ...
    if (!this.container) {
      this.container = NXT.utils.el('div', { id: 'toast-container', className: 'toast-container' });
      document.body.appendChild(this.container);
    }
  },

  show(message, type = 'info', duration = 4000) {
    if (this._active >= this.MAX_ACTIVE) {
...
NXT.modal = {
  ...
  init() {
    ...
    if (!this._overlay) {
      this._overlay = NXT.utils.el('div', { id: 'modal-overlay', className: 'modal-overlay' });
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape' && this._stack.length) this.close();
    });
  },
  ...
  close() {
    ...
    if (this._stack.length === 0) {
      ...
      setTimeout(() => { this._overlay.style.display = 'none'; }, 300);
...
NXT.auth = {
    ...
    NXT.modal.build('login', '🔐 Log in to Nextoken', `
      <form id="form-login" class="auth-form" novalidate>
        <div class="form-group">
          ...
          <span class="field-err" id="err-login-email"></span>
        </div>
        <div class="form-group">
          ...
          <span class="field-err" id="err-login-pass"></span>
        </div>
        <div class="form-row-inline">
          <label class="checkbox-label"><input type="checkbox" id="login-remember" /> Remember me</label>
      ...
      <form id="form-register" class="auth-form" novalidate>
        <div class="grid-2-sm">
          <div class="form-group">
            ...
            <input id="reg-first" type="text" placeholder="Jonas" required />
          </div>
          <div class="form-group">
            <label for="reg-last">Last name</label>
            <input id="reg-last" type="text" placeholder="Petrauskas" required />
          </div>
        </div>
        <div class="form-group">
          <label for="reg-email">Email address</label>
          <input id="reg-email" type="email" placeholder="you@example.com" autocomplete="email" required />
          <span class="field-err" id="err-reg-email"></span>
        </div>
        <div class="form-group">
          <label for="reg-country">Country of residence</label>
          <select id="reg-country" required>
            <option value="">Select country…</option>
            <option>Lithuania</option><option>Estonia</option><option>Latvia</option>
        ...
        <div class="form-group">
          ...
          <div class="input-eye">
            <input id="reg-pass" type="password" placeholder="Min. 12 characters" autocomplete="new-password" required />
        ...
        <label class="checkbox-label" style="margin-bottom:16px">
          ...
          I agree to the <a href="#" class="link-sm">Terms of Service</a> and <a href="#" class="link-sm">Privacy Policy</a>
        </label>
  ...
  switchTo(modal) {
    NXT.modal.close();
    setTimeout(() => NXT.modal.open(modal), 320);
  },
  ...
  _bindLoginForm() {
    ...
    form.addEventListener('submit', async e => {
      ...
      if (!email || !email.includes('@')) {
        document.getElementById('err-login-email').textContent = 'Enter a valid email address';
        return;
      }
      if (!pass || pass.length < 6) {
        document.getElementById('err-login-pass').textContent = 'Password must be at least 6 characters';
        return;
      }
      ...
      NXT.state.set('isLoggedIn', true);
      NXT.state.set('user', { email, name: email.split('@')[0] });
      NXT.state.set('kycLevel', 2);

      NXT.modal.close();
      NXT.toast.show(`Welcome back, ${NXT.state.user.name}! 👋`, 'success');
      NXT.auth._updateNavForLoggedIn();
  ...
  _bindRegisterForm() {
    ...
    form.addEventListener('submit', async e => {
      ...
      NXT.state.set('user', { name: first });
      NXT.state.set('kycLevel', 1);

      NXT.modal.close();
      NXT.toast.show(`Account created! Welcome, ${first} 🎉`, 'success');
      NXT.toast.show('Complete KYC to unlock trading', 'info', 6000);
      NXT.auth._updateNavForLoggedIn();
  ...
  _bindPasswordStrength() {
    ...
    input.addEventListener('input', () => {
      ...
      const colors = ['', '#e05050', '#f0a040', '#f0c040', '#38bd82'];
      meter.innerHTML = val.length
        ? `<div class="ps-bar"><div style="width:${score * 25}%;background:${colors[score]};height:3px;border-radius:2px;transition:all .3s"></div></div>
           <span style="font-size:0.72rem;color:${colors[score]}">${labels[score]}</span>`
        : '';
    });
  },
  ...
  _updateNavForLoggedIn() {
    ...
    if (loginBtn) {
      loginBtn.textContent = NXT.state.user?.name || 'Account';
      loginBtn.removeEventListener('click', loginBtn._handler);
      loginBtn._handler = () => NXT.router.go('dashboard');
      loginBtn.addEventListener('click', loginBtn._handler);
    }
  },

  logout() {
    NXT.state.set('isLoggedIn', false);
    NXT.state.set('user', null);
    NXT.state.set('kycLevel', 0);
    NXT.toast.show('You have been logged out', 'info');
  },
};
...
NXT.kyc = {
  ...
  render(containerId = 'kyc-widget') {
    ...
    if (!el) return;
    const level = NXT.state.kycLevel;
    const info  = this.LEVELS[level] || this.LEVELS[0];

    el.innerHTML = `
      <div class="kyc-status">
        <div class="kyc-indicator" style="background:${info.color}"></div>
        <div>
          <div class="kyc-label" style="color:${info.color}">${info.label}</div>
          <div class="kyc-desc">${info.desc}</div>
        </div>
        ${level < 3 ? `<button class="btn-sm" onclick="NXT.kyc.startVerification()">Upgrade →</button>` : '<span class="badge-verified">✓ Fully Verified</span>'}
      </div>
      <div class="kyc-steps">
        ${this.LEVELS.slice(1).map((l, i) => `
          <div class="kyc-step ${i < level ? 'done' : i === level - 1 ? 'current' : ''}">
            <div class="kyc-step-dot">${i < level ? '✓' : i + 1}</div>
            <span>${l.label.split(' — ')[1] || l.label}</span>
          </div>`).join('')}
      </div>`;
  },
  ...
  startVerification() {
    ...
    setTimeout(() => {
      NXT.state.set('kycLevel', Math.min(NXT.state.kycLevel + 1, 3));
      this.render();
    }, 2500);
  },
};
...
NXT.apiKeys = {
  ...
  render(container) {
    container.innerHTML = `
      <div class="akey-header">
        <h3>API Keys</h3>
        <button class="btn-primary btn-sm" id="btn-gen-key">+ Generate Key</button>
      </div>
      <div id="akey-list">
        ${this._keys.length
          ? this._keys.map(k => this._keyRow(k)).join('')
          : '<p style="color:var(--text-dim);font-size:.85rem">No API keys yet. Generate your first key to get started.</p>'}
      </div>
      <div class="akey-perms">
        <h4>Permissions</h4>
        <div class="perm-checkboxes">
          ${['Read Markets', 'Read Account', 'Trade', 'Withdraw', 'Tokenize'].map(p => `
            <label class="checkbox-label">
              <input type="checkbox" ${p !== 'Withdraw' ? 'checked' : ''} />
              ${p}
            </label>`).join('')}
        </div>
      </div>`;
  ...
  _keyRow(key) {
    return `
      <div class="akey-row" id="key-${key.id}">
        <div>
          <div class="akey-name">${key.name}</div>
          <div class="akey-val" title="Click to reveal">
            <code>${key.revealed ? key.value : key.masked}</code>
            <button class="btn-icon" onclick="NXT.apiKeys.toggle('${key.id}')">👁</button>
            <button class="btn-icon" onclick="NXT.apiKeys.copy('${key.id}')">⎘</button>
          </div>
          <div class="akey-meta">Created ${NXT.utils.timeAgo(key.created)} · Last used: ${key.lastUsed || 'Never'}</div>
        </div>
        <button class="btn-danger btn-sm" onclick="NXT.apiKeys.revoke('${key.id}')">Revoke</button>
      </div>`;
  },
  ...
  _generate(container) {
    ...
    const key = {
      id:       NXT.utils.uid('key'),
      name:     `Key ${this._keys.length + 1}`,
      value:    `nxt_live_${rand32()}`,
      masked:   `nxt_live_${'•'.repeat(32)}`,
      created:  new Date().toISOString(),
      lastUsed: null,
      revealed: false,
    };
    this._keys.push(key);
    NXT.toast.show(`API key "${key.name}" generated. Store it safely — it won't be shown again.`, 'success', 6000);
    this.render(container);
  },

  toggle(id) {
    const key = this._keys.find(k => k.id === id);
    if (key) { key.revealed = !key.revealed; this.render(document.getElementById('api-key-manager')); }
  },

  async copy(id) {
    const key = this._keys.find(k => k.id === id);
    if (!key) return;
    await NXT.utils.copyToClipboard(key.value);
    NXT.toast.show('API key copied to clipboard', 'success');
  },

  revoke(id) {
    this._keys = this._keys.filter(k => k.id !== id);
    NXT.toast.show('API key revoked', 'warn');
    this.render(document.getElementById('api-key-manager'));
  },
};
...
NXT.apiPlayground = {
  ...
  init() {
    const pg = document.getElementById('api-playground');
    if (!pg) return;
    ...
    NXT.utils.$$('.docs-content pre').forEach(pre => {
      ...
      btn.addEventListener('click', async () => {
        await NXT.utils.copyToClipboard(pre.textContent);
    ...
    NXT.utils.$$('.sidebar-link').forEach(link => {
      link.addEventListener('click', e => {
        e.preventDefault();
        const target = document.querySelector(link.getAttribute('href'));
        if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
        NXT.utils.$$('.sidebar-link').forEach(l => l.classList.remove('active'));
        link.classList.add('active');
      });
    });

    // IntersectionObserver to sync sidebar
  ...
  _buildTester() {
    ...
    container.innerHTML = `
      <div class="tester-header">
        <span class="eyebrow">Live API Tester</span>
      </div>
      <div class="tester-row">
        <select id="tester-method" class="tester-method">
          <option>GET</option><option>POST</option><option>DELETE</option>
        </select>
        <input id="tester-endpoint" type="text" value="/markets" placeholder="/markets" class="tester-input" />
        <button id="tester-send" class="btn-primary btn-sm">Send →</button>
      </div>
      <div class="tester-body-row" id="tester-body-row" style="display:none">
        <label>Request Body (JSON)</label>
        <textarea id="tester-body" class="tester-textarea" rows="6">{}</textarea>
      </div>
      <div class="tester-response" id="tester-response" style="display:none">
        <div class="tester-res-header">
          <span>Response</span>
          <span id="tester-status" class="tester-status"></span>
        </div>
        <pre id="tester-res-body" class="tester-res-body"></pre>
      </div>`;

    document.getElementById('tester-method')?.addEventListener('change', e => {
      const bodyRow = document.getElementById('tester-body-row');
      if (bodyRow) bodyRow.style.display = e.target.value !== 'GET' ? 'block' : 'none';
    });
  ...
  async _sendRequest() {
    ...
    const mockResponses = {
      '/markets':  { markets: Object.keys(NXT.market.assets).map(p => ({ pair: p, status: 'active', price: NXT.market.assets[p].price })) },
      '/ticker/BTC-EUR': { pair: 'BTC/EUR', price: NXT.market.assets['BTC/EUR'].price, change_24h: NXT.market.assets['BTC/EUR'].change?.toFixed(2) || '-3.66' },
      '/account/kyc': { level: NXT.state.kycLevel, status: ['none','pending','verified','enhanced'][NXT.state.kycLevel] },
      '/account/balance': { EUR: '5,000.00', BTC: '0.14230', ETH: '2.5012' },
    };
    ...
    if (method === 'DELETE') {
      responseData = { deleted: true, id: 'ord_' + Math.random().toString(36).slice(2,8) };
      statusCode   = 200;
    } else if (!NXT.state.isLoggedIn && endpoint !== '/markets') {
      responseData = { error: { code: 'UNAUTHORIZED', message: 'Missing or invalid API key.', status: 401 } };
      statusCode   = 401;
    } else if (mockKey) {
      responseData = mockResponses[mockKey];
      statusCode   = 200;
    } else if (method === 'POST') {
      responseData = { id: NXT.utils.uid(), status: 'created', created_at: new Date().toISOString() };
      statusCode   = 201;
    } else {
      responseData = { error: { code: 'NOT_FOUND', message: `Endpoint '${endpoint}' not found.`, status: 404 } };
      statusCode   = 404;
    }

    resEl.style.display = 'block';
    statusEl.textContent = statusCode;
    statusEl.className   = `tester-status ${statusCode < 300 ? 'ok' : statusCode < 500 ? 'warn' : 'err'}`;
    bodyEl.textContent   = JSON.stringify(responseData, null, 2);
...
NXT.blog = {
  ...
  init() {
    ...
    NXT.utils.$$('.cat-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        NXT.utils.$$('.cat-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        this.activeCategory = btn.textContent.trim();
        this._filterPosts();
      });
    });
    ...
    if (searchInput) {
      searchInput.addEventListener('input', NXT.utils.debounce(() => this._filterPosts(), 300));
    }

    // Read-more accordion (featured post)
    const featured = NXT.utils.$('.featured-post');
    if (featured) {
      featured.style.cursor = 'pointer';
      featured.addEventListener('click', () => {
        NXT.toast.show('Opening article…', 'info');
      });
    }
  },
  ...
  _filterPosts() {
    ...
    const posts  = NXT.utils.$$('.blog-card');

    posts.forEach(post => {
      const cat    = post.querySelector('.blog-cat')?.textContent.trim() || '';
      ...
      if (catMatch && queryMatch) {
        ...
        setTimeout(() => { if (!(catMatch && queryMatch)) post.style.display = 'none'; }, 250);
      }
    });
  },
};
...
NXT.careers = {
  ...
  init() {
    ...
    NXT.utils.$$('.apply-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const card = btn.closest('.job-card');
        const title = card?.querySelector('.job-title')?.textContent;
        this._openApplyModal(title || 'this role');
      });
    });

    // Job card → expand details
    NXT.utils.$$('.job-card').forEach(card => {
      card.addEventListener('click', e => {
        if (e.target.closest('.apply-btn')) return;
        card.classList.toggle('job-expanded');
      });
    });
  ...
  _filterJobs() {
    NXT.utils.$$('.job-card').forEach(card => {
      const dept = card.querySelector('.job-dept')?.textContent.trim() || '';
      const show = this._activeDept === 'All' || dept === this._activeDept;
  ...
  _buildApplyModal() {
    NXT.modal.build('apply', '📄 Apply for Position', `
      <form id="form-apply" class="auth-form" novalidate>
        <div class="form-group">
          ...
          <input id="apply-role" type="text" readonly style="opacity:.6;cursor:default" />
        </div>
        <div class="grid-2-sm">
          <div class="form-group">
            <label for="apply-first">First name</label>
            <input id="apply-first" type="text" placeholder="Jonas" required />
          </div>
          <div class="form-group">
            <label for="apply-last">Last name</label>
            <input id="apply-last" type="text" placeholder="Petrauskas" required />
          </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label for="apply-linkedin">LinkedIn / Portfolio URL</label>
          <input id="apply-linkedin" type="url" placeholder="https://linkedin.com/in/..." />
        </div>
        <div class="form-group">
          <label for="apply-cv">Upload CV (PDF, max 5MB)</label>
          <div class="file-drop" id="apply-cv-drop">
            <span>📎 Drop your CV here or <u>browse</u></span>
            <input type="file" id="apply-cv" accept=".pdf,.doc,.docx" style="display:none" />
          </div>
        </div>
        <div class="form-group">
          <label for="apply-cover">Cover letter (optional)</label>
          <textarea id="apply-cover" rows="4" placeholder="Tell us why you're excited about this role…"></textarea>
        </div>
    ...
    // File drop zone
    const dropZone = document.getElementById('apply-cv-drop');
    ...
    if (dropZone && fileInput) {
      ...
      dropZone.addEventListener('drop', e => {
        ...
        if (file) dropZone.querySelector('span').textContent = `✓ ${file.name}`;
      });
  ...
  _openApplyModal(role) {
    NXT.modal.open('apply');
    setTimeout(() => {
...
NXT.scrollAnimations = {
  ...
  refresh() {
    ...
    // Count-up elements
    NXT.utils.$$('[data-count]').forEach(el => {
      if (!el.dataset.observed) {
        el.dataset.observed = '1';
        this._obs?.observe(el);
      }
...
NXT.particles = {
  ...
  init(canvasId = 'bg-canvas') {
    ...
    window.addEventListener('resize', NXT.utils.throttle(() => {
      this._resize();
      this._generate();
    }, 400));

    // Mouse parallax
    document.addEventListener('mousemove', NXT.utils.throttle(e => {
  ...
  _generate() {
    this._particles = Array.from({ length: this.COUNT }, () => ({
      x:     NXT.utils.rand(0, this.canvas.width),
      y:     NXT.utils.rand(0, this.canvas.height),
      vx:    NXT.utils.rand(-0.12, 0.12),
      vy:    NXT.utils.rand(-0.12, 0.12),
      size:  NXT.utils.rand(0.8, 2.2),
      alpha: NXT.utils.rand(0.08, 0.28),
    }));
  },
  ...
  _draw() {
    ...
    this._particles.forEach(p => {
      ...
      ctx.fillStyle = `rgba(56,189,130,${p.alpha})`;
      ctx.fill();
    });
    ...
    for (let i = 0; i < this._particles.length; i++) {
      for (let j = i + 1; j < this._particles.length; j++) {
        ...
        if (dist < CONN_DIST) {
          ...
          ctx.strokeStyle = `rgba(56,189,130,${alpha})`;
          ctx.beginPath();
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
          ctx.stroke();
...
NXT.portfolio = {
  ...
  init() {
    ...
    NXT.market.subscribe(NXT.utils.throttle(() => this.render(container), 3000));
  },
  ...
  render(container) {
    ...
    const rows = this._holdings.map(h => {
      const price   = NXT.market.assets[h.pair]?.price || 0;
      ...
      return `
        <tr>
          <td><span class="asset-badge">${h.asset}</span></td>
          <td>${h.amount.toFixed(h.amount < 1 ? 4 : 2)}</td>
          <td>${NXT.utils.formatCurrency(price)}</td>
          <td>${NXT.utils.formatCurrency(value)}</td>
          <td class="${pnl >= 0 ? 'up' : 'down'}">
            ${NXT.utils.formatCurrency(pnl)} (${pnl >= 0 ? '+' : ''}${pnlPct.toFixed(2)}%)
          </td>
        </tr>`;
    }).join('');
    ...
    container.innerHTML = `
      <div class="port-summary">
        <div class="port-stat">
          ...
          <div class="port-stat-val">${NXT.utils.formatCurrency(totalValue)}</div>
        </div>
        <div class="port-stat">
          <div class="port-stat-label">Total P&L</div>
          <div class="port-stat-val ${totalPnl >= 0 ? 'up' : 'down'}">
            ${NXT.utils.formatCurrency(totalPnl)} (${totalPnl >= 0 ? '+' : ''}${totalPnlPct.toFixed(2)}%)
          </div>
        </div>
        <div class="port-stat">
          <div class="port-stat-label">Open Orders</div>
          <div class="port-stat-val">${NXT.state.openOrders.length}</div>
        </div>
      </div>
      <table class="port-table">
        <thead>
          <tr><th>Asset</th><th>Amount</th><th>Price</th><th>Value</th><th>P&L</th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>`;
  },
...
NXT.shortcuts = {
  ...
  init() {
    ...
    this.register('alt+l',  () => {
      if (NXT.state.isLoggedIn) NXT.auth.logout();
      else NXT.modal.open('login');
    }, 'Login / Logout');
    ...
    document.addEventListener('keydown', e => {
      ...
      const fn = this._map[key];
      if (fn) { e.preventDefault(); fn(); }
    });
  },

  register(combo, fn, label = '') {
    this._map[combo] = fn;
    this._map[combo]._label = label;
    this._map[combo]._combo = combo;
  },

  _showHelp() {
    const lines = Object.values(this._map)
      .filter(fn => fn._label)
      .map(fn => `<tr><td><kbd>${fn._combo}</kbd></td><td>${fn._label}</td></tr>`)
      .join('');

    NXT.modal.build('shortcuts', '⌨️ Keyboard Shortcuts', `
...
/* ═══════════════════════════════════════════════════════════
   22. INJECT SUPPORTING CSS (Runtime)
═══════════════════════════════════════════════════════════ */
...
NXT.styles = {
  inject() {
    const css = `
      ...
      .modal.modal-hide .modal-dialog { transform: translateY(20px) scale(.97); opacity: 0; }
      ...
      .form-group label { font-size: .82rem; color: var(--text-muted); font-weight: 400; }
      .form-group input, .form-group select, .form-group textarea {
        background: var(--surface); border: 1px solid var(--border); border-radius: 8px;
        padding: 10px 14px; color: var(--text); font-family: var(--font-body); font-size: .9rem;
      ...
      .checkbox-label input { width: 14px; height: 14px; accent-color: var(--accent); cursor: pointer; }
      ...
      /* ── Order Book ── */
      .ob-header { display: grid; grid-template-columns: 1fr 1fr 1fr; padding: 8px 12px; font-family: var(--font-mono); font-size: .68rem; color: var(--text-dim); text-transform: uppercase; letter-spacing: .06em; }
      ...
      .ob-mid small { font-size: .72rem; font-weight: 400; margin-left: 8px; font-family: var(--font-mono); }
      ...
      .tf-amount, .tf-time { color: var(--text-muted); }

      /* ── Copy Code Button ── */
      .copy-code-btn { position: absolute; top: 10px; right: 10px; background: var(--surface2); border: 1px solid var(--border); color: var(--text-muted); font-size: .72rem; font-family: var(--font-mono); padding: 4px 10px; border-radius: 5px; cursor: pointer; transition: all .2s; }
      .copy-code-btn:hover { color: var(--accent); border-color: var(--border-glow); }
      ...
      .tester-input:focus { outline: none; border-color: var(--border-glow); }
      .tester-body-row label { font-size: .78rem; color: var(--text-muted); display: block; margin-bottom: 6px; }
      ...
      .tester-response { margin-top: 14px; }
      ...
      .port-stat-label { font-size: .72rem; color: var(--text-dim); font-family: var(--font-mono); letter-spacing: .06em; text-transform: uppercase; margin-bottom: 4px; }
      ...
      .badge-verified { font-size: .75rem; color: var(--accent); border: 1px solid var(--border-glow); border-radius: 20px; padding: 4px 12px; white-space: nowrap; }
      ...
      .kyc-step.current { color: var(--text); }
      ...
      .kyc-step.current .kyc-step-dot { border-color: var(--text-muted); }

      /* ── API Key Manager ── */
      .akey-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; }
      .akey-header h3 { font-family: var(--font-display); font-weight: 700; font-size: 1rem; }
      ...
      .akey-name { font-weight: 500; font-size: .85rem; margin-bottom: 4px; }
      ...
      .akey-perms h4 { font-family: var(--font-display); font-weight: 700; font-size: .9rem; margin-bottom: 12px; }
      .perm-checkboxes { display: flex; flex-wrap: wrap; gap: 12px; }

      /* ── File Drop ── */
      .file-drop { border: 2px dashed var(--border); border-radius: 10px; padding: 20px; text-align: center; cursor: pointer; color: var(--text-muted); font-size: .85rem; transition: border-color .2s, background .2s; }
      ...
      .shortcut-table th { text-align: left; padding: 8px 12px; color: var(--text-dim); font-size: .72rem; font-family: var(--font-mono); text-transform: uppercase; border-bottom: 1px solid var(--border); }
...
/* ═══════════════════════════════════════════════════════════
   22. INIT — Bootstrap everything
═══════════════════════════════════════════════════════════ */

NXT.init = function() {
  // 1. Inject supporting CSS
  NXT.styles.inject();
  ...
  // 7. Particle background (if canvas present)
  NXT.particles.init('bg-canvas');

  // 8. Keyboard shortcuts
  NXT.shortcuts.init();

  // 9. Page-specific inits on page navigation
  NXT.state.on('page:exchange', () => {
    NXT.chart.init('price-chart');
    NXT.orderBook.init('order-book');
    NXT.tradeFeed.init('trade-feed');
    NXT.exchange.init();
    NXT.portfolio.init();
  });

  NXT.state.on('page:dashboard', () => {
    NXT.kyc.render('kyc-widget');
    NXT.portfolio.init();
    NXT.apiKeys.init();
    NXT.chart.init('dash-chart');
  });

  NXT.state.on('page:api', () => {
    NXT.apiPlayground.init();
  });

  NXT.state.on('page:careers', () => {
    NXT.careers.init();
  });

  NXT.state.on('page:blog', () => {
    NXT.blog.init();
  });

  // 10. Nav login/register button wiring
  document.querySelector('.nav-login')?.addEventListener('click', e => {
    e.preventDefault();
    NXT.modal.open(NXT.state.isLoggedIn ? 'register' : 'login');
  });
  document.querySelector('.nav-register')?.addEventListener('click', e => {
    e.preventDefault();
    NXT.modal.open('register');
  });

  // 11. Market pair switcher (if present)
  NXT.utils.$$('[data-market]').forEach(btn => {
    btn.addEventListener('click', () => {
      const pair = btn.dataset.market;
      NXT.state.set('activeMarket', pair);
      NXT.chart.setPair(pair);
      NXT.orderBook.setPair(pair);
      NXT.tradeFeed.pair = pair;
      NXT.utils.$$('[data-market]').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
    });
  });
  ...
  // 12. Welcome toast
  setTimeout(() => {
    NXT.toast.show('Welcome to Nextoken Capital — EU regulated tokenized markets', 'info', 5000);
  }, 800);
  ...
  console.log('%c JS Engine v2.1.0 loaded — %c NXT.state, NXT.market, NXT.router available globally ', 'color:#38bd82', 'color:#7a8599');
};

// Auto-init on DOMContentLoaded
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', NXT.init);
} else {
  NXT.init();
}

            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
              ...
              <div style={{ position: 'relative' }}>
                <select
                  ...
                  style={{
                    appearance: 'none', background: '#141414', border: '1px solid #2a2a2a',
                ...
                <span style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', color: '#f5c842', pointerEvents: 'none', fontSize: '12px' }}>▼</span>
              </div>
              ...
              {selectedAsset && (
                <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: '#4ade80', fontSize: '16px', fontWeight: '700' }}>{selectedAsset.annualYield ? `${selectedAsset.annualYield}%` : '—'}</div>
                  </div>
                  <div>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '16px', flexWrap: 'wrap' }}>
                {[...new Set(assets.map(a => a.assetType).filter(Boolean))].map(type => (
                  <button key={type} onClick={() => {
                    ...
                    style={{
                      background: selectedAsset?.assetType === type ? '#f5c842' : '#141414',
        ...
        {!selectedAsset ? (
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 16px', display: 'grid', gridTemplateColumns: '1fr 360px', gap: '16px', paddingBottom: '40px', paddingTop: '16px' }}>
            ...
            <div>
              ...
              {/* Price chart area */}
              <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <span style={{ fontWeight: '700', fontSize: '15px', color: '#fff' }}>Price History</span>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {['1D', '7D', '30D', '90D'].map(r => (
                      <button key={r} style={{ background: r === '7D' ? '#f5c842' : '#141414', color: r === '7D' ? '#000' : '#555', border: '1px solid #222', borderRadius: '6px', padding: '4px 10px', cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>{r}</button>
                    ))}
                  </div>
                </div>
                <div style={{ height: '180px', background: '#080808', borderRadius: '8px', border: '1px solid #161616', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
              ...
              <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ display: 'flex', borderBottom: '1px solid #1a1a1a' }}>
                  {[
                    ...
                    <button key={t.key} onClick={() => setTab(t.key)} style={{
                      ...
                      color: tab === t.key ? '#f5c842' : '#555', cursor: 'pointer',
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                    ...
                    <div style={{ padding: '16px', borderRight: '1px solid #1a1a1a' }}>
                      ...
                      {orderBook.bids.length === 0
                        ? <div style={{ color: '#2a2a2a', fontSize: '13px', paddingTop: '24px', textAlign: 'center' }}>No bids yet</div>
                        : orderBook.bids.map((b, i) => (
                          <div key={i} onClick={() => { setPrice(b.price.toString()); setSide('sell'); }}
                            ...
                            <span style={{ color: '#4ade80', fontSize: '13px', fontWeight: '600' }}>{b.price}</span>
                    ...
                    <div style={{ padding: '16px' }}>
                      <div style={{ color: '#f87171', fontWeight: '700', fontSize: '12px', letterSpacing: '1px', marginBottom: '14px' }}>▼ ASKS (Sellers)</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', marginBottom: '8px' }}>
                        {['Price €', 'Units', 'Value €'].map(h => <span key={h} style={{ color: '#333', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</span>)}
                      </div>
                      {orderBook.asks.length === 0
                        ? <div style={{ color: '#2a2a2a', fontSize: '13px', paddingTop: '24px', textAlign: 'center' }}>No asks yet</div>
                        : orderBook.asks.map((a, i) => (
                          <div key={i} onClick={() => { setPrice(a.price.toString()); setSide('buy'); }}
                            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '6px 0', borderBottom: '1px solid #111', cursor: 'pointer', borderRadius: '4px' }}>
                            <span style={{ color: '#f87171', fontSize: '13px', fontWeight: '600' }}>{a.price}</span>
                ...
                {tab === 'trades' && (
                  <div style={{ padding: '16px' }}>
                    {trades.length === 0
                      ? <div style={{ color: '#2a2a2a', textAlign: 'center', padding: '40px', fontSize: '14px' }}>No trades executed yet</div>
                      : trades.map((t, i) => (
                        <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', padding: '8px 0', borderBottom: '1px solid #111', fontSize: '13px' }}>
                          ...
                          <span style={{ color: t.side === 'buy' ? '#4ade80' : '#f87171', fontWeight: '600', textTransform: 'uppercase' }}>{t.side || 'TRADE'}</span>
                          <span style={{ color: '#aaa' }}>{t.units} units</span>
                          <span style={{ color: '#f5c842', fontWeight: '600' }}>€{t.price}</span>
                        </div>
                      ))}
                  </div>
                )}
            ...
            <div style={{ position: 'sticky', top: '88px', height: 'fit-content' }}>
              <div style={{ background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '20px' }}>
                ...
                {/* Limit / Market toggle */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginBottom: '16px', background: '#0a0a0a', padding: '4px', borderRadius: '10px', border: '1px solid #1a1a1a' }}>
                  {['limit', 'market'].map(t => (
                    <button key={t} onClick={() => setOrderType(t)} style={{
                      background: orderType === t ? '#1e1e1e' : 'none', border: orderType === t ? '1px solid #2a2a2a' : '1px solid transparent',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '20px' }}>
                  <button onClick={() => setSide('buy')} style={{
                    background: side === 'buy' ? 'linear-gradient(135deg,#16a34a,#15803d)' : '#111',
                    border: `1px solid ${side === 'buy' ? '#16a34a' : '#1f1f1f'}`,
                  ...
                  <button onClick={() => setSide('sell')} style={{
                    background: side === 'sell' ? 'linear-gradient(135deg,#dc2626,#b91c1c)' : '#111',
                    border: `1px solid ${side === 'sell' ? '#dc2626' : '#1f1f1f'}`,
                    color: '#fff', borderRadius: '10px', padding: '13px', cursor: 'pointer',
                    fontWeight: '800', fontSize: '15px', letterSpacing: '0.5px', transition: 'all 0.2s'
                  }}>SELL</button>
                </div>
                ...
                <form onSubmit={placeOrder}>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        <input type="number" step="0.01" value={price} onChange={e => setPrice(e.target.value)}
                          ...
                          style={{ width: '100%', background: '#0a0a0a', border: '1px solid #222', color: '#fff', padding: '12px 12px 12px 28px', borderRadius: '8px', fontSize: '15px', boxSizing: 'border-box', outline: 'none' }} />
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ marginBottom: '14px' }}>
                    ...
                    <input type="number" step="1" min="1" value={units} onChange={e => setUnits(e.target.value)}
                      placeholder="0"
                      style={{ width: '100%', background: '#0a0a0a', border: '1px solid #222', color: '#fff', padding: '12px', borderRadius: '8px', fontSize: '15px', boxSizing: 'border-box', outline: 'none' }} />
                    {/* Quick picks */}
                    <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                      {['1', '5', '10', '25'].map(n => (
                        <button key={n} type="button" onClick={() => setUnits(n)}
                          style={{ flex: 1, background: '#111', border: '1px solid #1f1f1f', color: '#666', borderRadius: '6px', padding: '5px 0', cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>{n}</button>
                      ))}
                    </div>
                  </div>

                  {/* Summary box */}
                  <div style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '8px', padding: '14px', marginBottom: '16px' }}>
                    ...
                    <div style={{ borderTop: '1px solid #1a1a1a', paddingTop: '8px', display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                      ...
                      <span style={{ color: '#f5c842', fontWeight: '800' }}>€{(parseFloat(total) + parseFloat(fee)).toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <button type="submit" disabled={loading || !units}
                    style={{
                      width: '100%',
                      background: !units ? '#111' : side === 'buy' ? 'linear-gradient(135deg,#16a34a,#15803d)' : 'linear-gradient(135deg,#dc2626,#b91c1c)',
                ...
                {msg && (
                  <div style={{ marginTop: '12px', padding: '12px', background: msg.startsWith('✅') ? '#052e16' : '#2d0a0a', border: `1px solid ${msg.startsWith('✅') ? '#166534' : '#7f1d1d'}`, borderRadius: '8px', fontSize: '13px', color: msg.startsWith('✅') ? '#4ade80' : '#f87171', textAlign: 'center', fontWeight: '600' }}>
                    {msg}
                  </div>
                )}

                {/* Asset details */}
                <div style={{ marginTop: '16px', background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '8px', padding: '14px' }}>
                ...
                <div style={{ color: '#2a2a2a', fontSize: '11px', textAlign: 'center', marginTop: '12px', lineHeight: '1.5' }}>

...
export default function ForgotPassword() {
  ...
  return (
    <>
      <Head><title>{t("forgot_password.forgot_password_nextoken_capital")}</title></Head>
      <div style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"#0B0E11" }}>
        <div style={{ width:"100%",maxWidth:400,background:"#0F1318",border:"1px solid rgba(255,255,255,0.08)",borderRadius:16,padding:32 }}>
          <div style={{ textAlign:"center",marginBottom:24 }}>
            ...
            <div style={{ fontSize:11,color:"rgba(255,255,255,0.3)",letterSpacing:1 }}>{t("forgot_password.nextoken_capital")}</div>
          </div>

          {sent ? (
            <div style={{ textAlign:"center" }}>
              <div style={{ fontSize:48,marginBottom:16 }}>📧</div>
              <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:8 }}>{t("forgot_password.check_your_email")}</h2>
              ...
              <button onClick={() => router.push("/login")} style={{ width:"100%",padding:12,background:"#F0B90B",color:"#000",fontSize:14,fontWeight:700,border:"none",borderRadius:8,cursor:"pointer",fontFamily:"inherit" }}>{t("forgot_password.back_to_login")}</button>
            </div>
          ) : (
            <>
              <h2 style={{ fontSize:18,fontWeight:800,color:"#fff",marginBottom:4 }}>{t("forgot_password.forgot_password")}</h2>
              <p style={{ fontSize:13,color:"rgba(255,255,255,0.4)",marginBottom:20 }}>{t("forgot_password.enter_your_email_and_we_will_send_a_rese")}</p>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom:16 }}>
                  <label style={{ display:"block",fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:.5,marginBottom:6 }}>{t("forgot_password.email")}</label>
              ...
              <div style={{ textAlign:"center",marginTop:16 }}>
                <button onClick={() => router.push("/login")} style={{ background:"none",border:"none",color:"#F0B90B",fontSize:13,cursor:"pointer",fontFamily:"inherit" }}>{t("forgot_password.back_to_login_2")}</button>
              </div>
            </>
          )}
        </div>

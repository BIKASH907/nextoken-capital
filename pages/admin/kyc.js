...
const COUNTRIES = [
  ...
  'Bahamas','Bahrain','Bangladesh','Belarus','Belgium','Belize','Bhutan','Bolivia','Bosnia and Herzegovina',
  'Botswana','Brazil','Bulgaria','Cambodia','Cameroon','Canada','Chile','China','Colombia','Croatia','Cuba',
  'Cyprus','Czech Republic','Denmark','Ecuador','Egypt','Estonia','Ethiopia','Finland','France','Georgia',
  'Germany','Ghana','Greece','Guatemala','Honduras','Hungary','Iceland','India','Indonesia','Iran','Iraq',
  'Ireland','Israel','Italy','Jamaica','Japan','Jordan','Kazakhstan','Kenya','Kuwait','Latvia','Lebanon',
  'Libya','Liechtenstein','Lithuania','Luxembourg','Malaysia','Malta','Mexico','Moldova','Monaco','Mongolia',
  'Montenegro','Morocco','Myanmar','Nepal','Netherlands','New Zealand','Nigeria','North Macedonia','Norway',
  ...
  'Sri Lanka','Sudan','Sweden','Switzerland','Syria','Taiwan','Thailand','Turkey','Ukraine',
...
const KYB_CHECKLIST = [
  ...
  {
    section: 'Ultimate Beneficial Owners (UBOs)',
    items: [
  ...
  {
    section: 'Directors & Authorized Signatories',
    items: [
...
export default function KYCAdmin() {
  ...
  async function load() {
    ...
    try {
      if (mode === 'kyb') {
        const res  = await fetch('/api/admin/kyb?status=all');
        ...
        users.forEach(u => { const s = u.kycStatus || 'pending'; if (c[s] !== undefined) c[s]++; });
        setCounts(c);
      }
    } catch (e) {}
    setLoading(false);
  ...
  const items = allItems.filter(item => {
    ...
    const country= (isKYB ? item.userId?.country || item.incorporationCountry : item.country) || '';
  ...
  function toggleCheck(section, item) {
    const key = section + '::' + item;
    setChecklist(prev => ({ ...prev, [key]: !prev[key] }));
  }

  const statuses = ['pending','in_review','approved','rejected'];
  const inp = { background:'#0d0d0d', border:'1px solid #222', color:'#fff', padding:'9px 13px', borderRadius:'8px', fontSize:'13px', outline:'none' };
  ...
  function DocModal({ record, onClose }) {
    ...
    return (
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
        <div style={{ background:'#0f0f0f', border:'1px solid #2a2a2a', borderRadius:'14px', width:'100%', maxWidth:'660px', maxHeight:'85vh', overflow:'auto', padding:'24px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'20px' }}>
            <div>
              ...
              <div style={{ color:'#555', fontSize:'12px', marginTop:'4px' }}>
                {mode === 'kyb' ? 'KYB Business Verification Documents' : 'KYC Identity Documents'}
              </div>
            </div>
          ...
          <div style={{ background:'#141414', borderRadius:'10px', padding:'16px', marginBottom:'16px' }}>
            <div style={{ color:'#444', fontSize:'10px', letterSpacing:'1.5px', textTransform:'uppercase', marginBottom:'12px', fontWeight:'700' }}>Profile</div>
            {[
              ...
              ['Type',      record.userId?.accountType || record.accountType || '—'],
          ...
          {docs.length === 0 ? (
            <div style={{ background:'#141414', borderRadius:'10px', padding:'32px', textAlign:'center', color:'#333' }}>
              <div style={{ fontSize:'32px', marginBottom:'8px' }}>📂</div>No documents uploaded yet
            </div>
          ) : docs.map((doc, i) => (
          ...
          {/* Sumsub */}
          {(record.sumsubApplicantId || record.userId?.sumsubApplicantId) && (
            <div style={{ background:'#0a1628', border:'1px solid #1e3a5f', borderRadius:'8px', padding:'14px', marginTop:'12px' }}>
              <div style={{ color:'#60a5fa', fontWeight:'700', fontSize:'13px', marginBottom:'4px' }}>🔍 Sumsub Verification</div>
              <div style={{ color:'#888', fontSize:'12px' }}>ID: {record.sumsubApplicantId || record.userId?.sumsubApplicantId}</div>
            </div>
          )}
          ...
          <div style={{ display:'flex', gap:'10px', marginTop:'20px' }}>
            {['in_review','approved','rejected'].map(s => {
              ...
              return (
                <button key={s} onClick={() => { updateStatus(record._id, s); onClose(); }} style={{
                  background:sc.bg, color:sc.color, border:'1px solid '+sc.border,
                  borderRadius:'8px', padding:'10px 18px', cursor:'pointer', fontWeight:'700', fontSize:'13px'
                }}>{s === 'in_review' ? '🔍 Review' : s === 'approved' ? '✅ Approve' : '❌ Reject'}</button>
              );
            })}
          </div>
        </div>
    ...
    <AdminLayout>
      ...
      <div style={{ padding:'28px 32px', background:'#080808', minHeight:'100vh' }}>
        ...
        <div style={{ display:'flex', gap:'12px', marginBottom:'24px' }}>
          {[['kyc','🪪 KYC (Individuals)'],['kyb','🏛️ KYB (Businesses / Issuers)']].map(([m,label]) => (
            <button key={m} onClick={() => { setMode(m); setFilterStatus('all'); setSearch(''); setFilterCountry(''); setFilterGender(''); }} style={{
              background: mode === m ? '#f5c842' : '#141414', color: mode === m ? '#000' : '#888',
              border:'1px solid #2a2a2a', borderRadius:'8px', padding:'10px 20px',
              cursor:'pointer', fontWeight:'700', fontSize:'13px'
            }}>{label}</button>
          ))}
          <button onClick={() => setShowChecklist(!showChecklist)} style={{
            background: showChecklist ? '#1e3a5f' : '#141414', color: showChecklist ? '#60a5fa' : '#888',
            border:'1px solid #2a2a2a', borderRadius:'8px', padding:'10px 20px',
            cursor:'pointer', fontWeight:'700', fontSize:'13px', marginLeft:'auto'
          }}>📋 KYB Checklist</button>
        </div>

        {/* KYB CHECKLIST PANEL */}
        {showChecklist && (
          <div style={{ background:'#0f0f0f', border:'1px solid #1a1a1a', borderRadius:'12px', padding:'24px', marginBottom:'24px' }}>
            <div style={{ fontWeight:'800', fontSize:'16px', color:'#fff', marginBottom:'6px' }}>KYB Verification Checklist</div>
            <div style={{ color:'#555', fontSize:'13px', marginBottom:'20px' }}>Track completion of required documents per applicant</div>

            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'24px' }}>
              {KYB_CHECKLIST.map(group => (
                <div key={group.section} style={{ background:'#141414', borderRadius:'10px', padding:'16px' }}>
                  <div style={{ color:'#f5c842', fontSize:'11px', fontWeight:'800', letterSpacing:'1.2px', textTransform:'uppercase', marginBottom:'14px' }}>
                    {group.section}
                  </div>
                  {group.items.map(item => {
                    ...
                    return (
                      <label key={item} style={{ display:'flex', alignItems:'flex-start', gap:'10px', marginBottom:'10px', cursor:'pointer' }}>
                        <div onClick={() => toggleCheck(group.section, item)} style={{
                          width:'16px', height:'16px', minWidth:'16px', borderRadius:'4px', marginTop:'2px',
                        ...
                        <span style={{ color: done ? '#888' : '#aaa', fontSize:'13px', lineHeight:'1.4', textDecoration: done ? 'line-through' : 'none' }}>{item}</span>
                      </label>
                    );
                  })}
                </div>
        ...
        {/* ── SEARCH + FILTERS ROW ── */}
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr', gap:'10px', marginBottom:'16px', alignItems:'center' }}>
          {/* Search */}
          <div style={{ position:'relative' }}>
            ...
            <input
              ...
              style={{ ...inp, width:'100%', paddingLeft:'36px', boxSizing:'border-box' }}
            />
          </div>
          ...
          {/* Gender */}
          <select value={filterGender} onChange={e => setFilterGender(e.target.value)} style={inp}>
          ...
          {/* Clear filters */}
          <button onClick={() => { setSearch(''); setFilterCountry(''); setFilterGender(''); setFilterStatus('all'); }} style={{
            background:'#141414', color:'#666', border:'1px solid #222',
            borderRadius:'8px', padding:'9px 14px', cursor:'pointer', fontSize:'13px', fontWeight:'600'
          }}>✕ Clear</button>
        </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} style={{ padding:'48px', textAlign:'center', color:'#333' }}>
                ...
                return (
                  <tr key={item._id || i}
                    style={{ borderBottom:'1px solid #111', background: selected?._id === item._id ? '#111' : 'transparent', cursor:'pointer' }}
                    ...
                    <td style={{ padding:'13px 16px' }}>
                      <span style={{ background:sc.bg, color:sc.color, border:'1px solid '+sc.border, borderRadius:'6px', padding:'3px 10px', fontSize:'11px', fontWeight:'700', textTransform:'capitalize' }}>
                        {status.replace('_',' ')}
                      </span>
                    </td>
          ...
          <div style={{ background:'#0f0f0f', border:'1px solid #2a2a2a', borderRadius:'12px', padding:'20px', marginTop:'16px' }}>
            ...
            <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
              ...
              {['in_review','approved','rejected'].map(s => {
                const sc = STATUS_COLORS[s];
                return (
                  <button key={s} onClick={() => updateStatus(selected._id, s)} style={{ background:sc.bg, color:sc.color, border:'1px solid '+sc.border, borderRadius:'8px', padding:'10px 18px', cursor:'pointer', fontWeight:'700', fontSize:'13px', textTransform:'capitalize' }}>

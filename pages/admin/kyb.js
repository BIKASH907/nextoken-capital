...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    let records = [];
    try {
      records = await KYB.find(kybQuery)
    ...
    const missing = issuers.filter(u => !existing.includes(u._id.toString()));

    for (const u of missing) {
      try {
        const stub = await KYB.create({
          userId: u._id,
          companyName: u.companyName || u.name || 'Unnamed',
          status: 'pending',
          submittedAt: u.createdAt || new Date(),
          autoCreated: true
        });
        records.push({ ...stub.toObject(), userId: u });
      } catch (e) {}
    }

...
export default function AdminDashboard() {
  ...
  const handleImagePick = async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const d = await r.json();
        if (d.url) {
          setImages(prev => prev.map(img => img.tempId === tempId ? { ...img, url: d.url, uploading: false } : img));
        } else {
          setImages(prev => prev.filter(img => img.tempId !== tempId));
          setAssetMsg("Upload failed: " + (d.error || "unknown"));
        }
      } catch(err) {
        setImages(prev => prev.filter(img => img.tempId !== tempId));
        setAssetMsg("Upload error");
      }
    }
    setUploading(false);
    e.target.value = "";
  ...
  const createAsset = async (e) => {
    ...
    try {
      const payload = { ...assetForm, images: images.filter(i => i.url).map(i => i.url) };
                ...
                {assets.map(a => (
                  <div className="asset-card" key={a._id}>
                    ...
                    <div className="asset-card-body">
                      ...
                      <div className="asset-card-row"><span>Photos</span><span>{a.images?.length || 0}</span></div>
            ...
            <div className="card">
              <form onSubmit={createAsset}>
                ...
                <div className="field" style={{marginTop:16}}>
                  <label>Asset Photos ({images.length} added)</label>
                  {images.length > 0 && (
                  ...
                  <div className="upload-zone" onClick={() => fileRef.current.click()}>
                    ...
                    <div style={{fontSize:13,color:"rgba(255,255,255,0.4)"}}>{uploading ? "Uploading..." : "Click to add photos (multiple allowed)"}</div>
                  </div>
                </div>

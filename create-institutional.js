...
const S = {
  ...
  sub:   { fontSize:16, color:"#8a9bb8", fontWeight:300, maxWidth:600, lineHeight:1.75, margin:"0 0 40px" },
...
const regions = [
  { flag:"🇱🇹", name:"European Union",    note:"MiCA regulated · VASP registered · Bank of Lithuania supervised",  color:"#818cf8" },
  { flag:"🇸🇬", name:"Singapore",         note:"MAS-compatible structure · APAC gateway for tokenized RWA",         color:"#22c55e" },
  ...
  { flag:"🇨🇭", name:"Switzerland",       note:"FINMA compatible · DLT Act jurisdiction · Crypto Valley access",    color:"#f87171" },
  ...
  { flag:"🇭🇰", name:"Hong Kong",         note:"SFC licensed pathway · Type 1 & 7 regulated activities",            color:"#4ade80" },
];
...
function FaqItem({ q, a }) {
  ...
  return (
    <div style={{ border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, overflow:"hidden", marginBottom:8 }}>
      <button onClick={() => setOpen(!open)} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"18px 20px", background:"#0d0d14", border:"none", color:"#e8e8f0", fontSize:15, fontWeight:500, cursor:"pointer", textAlign:"left", fontFamily:"inherit" }}>
        <span>{q}</span>
        <span style={{ color:"#F0B90B", fontSize:22, flexShrink:0, marginLeft:16 }}>{open ? "−" : "+"}</span>
      </button>
      {open && <div style={{ padding:"0 20px 20px", background:"#0d0d14" }}><p style={{ fontSize:14, color:"#8a9bb8", lineHeight:1.75, margin:0 }}>{a}</p></div>}
    </div>
  );
}
    ...
    <div style={S.page}>
      ...
      <div style={{ margin:"0 32px", borderTop:"1px solid rgba(255,255,255,0.07)", borderBottom:"1px solid rgba(255,255,255,0.07)", background:"#0d0d14", display:"flex", flexWrap:"wrap" }}>
        {stats.map((s,i,arr) => (
          <div key={s.l} style={{ flex:1, minWidth:130, padding:"24px 20px", textAlign:"center", borderRight:i<arr.length-1?"1px solid rgba(255,255,255,0.07)":"none" }}>
            ...
            <div style={{ fontSize:11.5, color:"#8a9bb8", marginTop:4 }}>{s.l}</div>
          </div>
        ))}
      </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16 }}>
          {useCases.map((c) => (
            <div key={c.title}
              ...
              <h3 style={{ fontFamily:"Syne,sans-serif", fontSize:16, fontWeight:700, color:"#e8e8f0", marginBottom:8 }}>{c.title}</h3>
              <p style={{ fontSize:13.5, color:"#8a9bb8", lineHeight:1.7 }}>{c.desc}</p>
            </div>
          ))}
        </div>
      </div>
            ...
            {regions.map((r) => (
              <div key={r.name} style={{ ...S.card, display:"flex", alignItems:"center", gap:16 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:12, color:"#8a9bb8", lineHeight:1.6 }}>{r.note}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={S.sec}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:20 }}>
          {tiers.map((t) => (
            <div key={t.name} style={{ ...S.card, border: t.featured ? "1px solid rgba(240,185,11,0.4)" : "1px solid rgba(255,255,255,0.07)", background: t.featured ? "linear-gradient(135deg,rgba(240,185,11,0.06) 0%,#0d0d14 100%)" : "#0d0d14", position:"relative" }}>
              ...
              <div style={{ fontSize:13, color:"#8a9bb8", marginBottom:20 }}>Min. AUM: <strong style={{ color:"#e8e8f0" }}>{t.minAUM}</strong></div>
              <div style={{ marginBottom:24 }}>
                {t.features.map((f) => (
                  <div key={f} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <span style={{ fontSize:13.5, color:"#b0b7c3" }}>{f}</span>
                  </div>
                ))}
              </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
            {[
              ...
              <div key={c.title} style={{ ...S.card }}>
                ...
                <p style={{ fontSize:13, color:"#8a9bb8", lineHeight:1.65 }}>{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ position:"relative", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:32, marginTop:16 }}>
          ...
          {[
            ...
            <div key={s.n}>
              ...
              <p style={{ fontSize:12.5, color:"#8a9bb8", lineHeight:1.65, margin:0 }}>{s.b}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <footer style={{ borderTop:"1px solid rgba(255,255,255,0.07)", padding:"48px 32px 28px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          <div style={{ borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:20, display:"flex", flexWrap:"wrap", justifyContent:"space-between", gap:10 }}>
            ...
            <p style={{ fontSize:11, color:"#8a9bb8", opacity:0.6, margin:0 }}>Institutional services are subject to eligibility, jurisdiction, and regulatory approval.</p>
          </div>
        </div>
      </footer>
    </div>

...
echo "    if (!client) return NextResponse.json({ success: false, error: 'DB connection failed' }, { status: 500 });" >> app/api/tokenize/route.ts
...
echo "    return NextResponse.json({ success: true, message: 'Asset saved successfully', id: result.insertedId }, { status: 201 });" >> app/api/tokenize/route.ts
